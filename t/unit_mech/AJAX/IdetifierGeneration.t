...
ok($new_list_id);
is_deeply($response, {'success' => 'Stored test_identifier_generation!','new_list_id' => 16}, 'test create identifier generation list entry');

$mech->get_ok('http://localhost:3010/ajax/breeders/identifier_generation_list');
$response = decode_json $mech->content;
print STDERR Dumper $response;
is_deeply($response, {'data' => [['test_identifier_generation','test','ACTLNG','6',1,'ACTLNG000001','<button class="btn btn-primary" name="identifier_generation_history" data-list_id="16">View</button>','<div class="form-group"><label class="col-sm-6 control-label">Next Count: </label><div class="col-sm-6"> <input type="number" class="form-control" id="identifier_generation_next_numbers_16" placeholder="EG: 100" /></div></div><button class="btn btn-primary" name="identifier_generation_download" data-list_id="16">Download Next</button>']]}, 'test identifier generation list');
