...
sub _validate_with_plugin {
    ...
    for my $row ( 1 .. $row_max ) {
        ...
        if ($current_trial_name && $current_trial_name ne $trial_name) {
            ...
            if ($worksheet->get_cell($row,1)) {
                $company = $worksheet->get_cell($row,1)->value();
            } else {
                $company = undef;
            }

            if ($worksheet->get_cell($row,2)) {
                $breeding_program = $worksheet->get_cell($row,2)->value();
            } else {
        ...
        if ($working_on_new_trial) {
            ...
            $h_comp->execute($company);
            my ($company_id) = $h_comp->fetchrow_array();

            if (!$company || $company eq '' ) {
                push @error_messages, "Cell B$row_name: company missing.";
            }
            elsif (!$company_id) {
                push @error_messages, "Cell B$row_name: company does not exist in database.";
            }
            elsif (!exists($allowed_private_company_ids_hash->{$company_id})) {
                push @error_messages, "Cell B$row_name: you are not part of this company.";
            }
            elsif ($allowed_private_company_access_hash->{$company_id} ne 'submitter_access' && $allowed_private_company_access_hash->{$company_id} ne 'curator_access') {
                push @error_messages, "Cell B$row_name: you do not have the required company access.";
            }
            else {
                $company =~ s/^\s+|\s+$//g; #trim whitespace from front and end...
                $seen_companies{$company}=$row_name;
            }

            ## BREEDING PROGRAM CHECK
            if (!$breeding_program || $breeding_program eq '' ) {
                push @error_messages, "Cell C$row_name: breeding_program missing.";
            }
            else {
            ...
            if (!$location || $location eq '' ) {
                push @error_messages, "Cell D$row_name: location missing.";
            }
            else {
            ...
            ## YEAR CHECK
            if (!($year =~ /^\d{4}$/)) {
                push @error_messages, "Cell E$row_name: <b>$year</b> is not a valid year, must be a 4 digit positive integer.";
            }
        ...
        if (!$plot_name || $plot_name eq '' ) {
            ...
            if ($seen_plot_names{$plot_name}) {
                push @error_messages, "Cell N$row_name: duplicate plot name <b>$plot_name</b> seen before at cell N".$seen_plot_names{$plot_name}.".";
            }
        ...
        foreach my $treatment_name (@treatment_names){
            if($worksheet->get_cell($row,$treatment_col)){
                ...
                if (defined($apply_treatment) && $apply_treatment ne '' && $apply_treatment ne '1'){
    ...
    ## COMPANY NAMES OVERALL VALIDATION
    my @company_names = keys %seen_companies;
    my $companies_missing = $validator->validate($schema,'private_companies',\@company_names)->{'missing'};
    my @companies_missing = @{$companies_missing};
    if (scalar(@companies_missing) > 0) {
        push @error_messages, "Company(s) <b>".join(',',@companies_missing)."</b> are not in the database.";
    }
    ...
    while (my $r=$rs->next){
        push @error_messages, "Cell N".$seen_plot_names{$r->uniquename}.": plot name <b>".$r->uniquename."</b> already exists.";
    }
...
sub _parse_with_plugin {
    ...
    for my $row ( 1 .. $row_max ) {
        ...
        if ($current_trial_name && $current_trial_name ne $trial_name) {
            ...
            $single_design{'private_company_id'} = $company_id;
            $single_design{'breeding_program'} = $worksheet->get_cell($row,2)->value();
            $single_design{'location'} = $worksheet->get_cell($row,3)->value();
...
sub _parse_header {
    ...
    if ($worksheet->get_cell(0,1)) {
        $company_head= $worksheet->get_cell(0,1)->value();
    }
    if ($worksheet->get_cell(0,2)) {
        $breeding_program_head= $worksheet->get_cell(0,2)->value();
    }
    if ($worksheet->get_cell(0,3)) {
        $location_head= $worksheet->get_cell(0,3)->value();
    }
