...
sub observationunits_update {
    ...
    foreach my $params (@$data) {
        ...
        if (!$stock) {
            return CXGN::BrAPI::JSONResponse->return_error($self->status, "observationUnitDbId not found: $observation_unit_db_id", 400);
        }
...
sub observationunits_store {
    ...
    foreach my $params (@{$data}) {
        ...
        if (!$plot_number){
            return CXGN::BrAPI::JSONResponse->return_error($self->status, sprintf('Provide a sequential plot number for unique for all observation units in the study.'));
        }
