...
sub store {
    ...
    if (!$user_id){
        return CXGN::BrAPI::JSONResponse->return_error($self->status, sprintf('You must be logged in to add a study!'));
    }
...
sub _save_trial {
    ...
    my $project = $chado_schema->resultset('Project::Project')->create({
        name => $trial_name,
