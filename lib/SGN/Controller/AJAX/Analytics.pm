...
sub analytics_protocols_compare_to_trait :Path('/ajax/analytics_protocols_compare_to_trait') Args(0) {
    ...
    eval {
        my $result_blup_data_s_rep1;
        my $genetic_effect_max_s_rep1 = -1000000000;
        my $genetic_effect_min_s_rep1 = 10000000000;
        my $genetic_effect_sum_square_s_rep1 = 0;
        my $genetic_effect_sum_s_rep1 = 0;
        my $result_blup_spatial_data_s_rep1;
        my $env_effect_min_s_rep1 = 100000000;
        my $env_effect_max_s_rep1 = -100000000;
        my $env_effect_sum_s_rep1 = 0;
        my $env_effect_sum_square_s_rep1 = 0;
        my $result_residual_data_s_rep1;
        my $result_fitted_data_s_rep1;
        my $residual_sum_s_rep1 = 0;
        my $residual_sum_square_s_rep1 = 0;
        my $model_sum_square_residual_s_rep1 = 0;
        my @varcomp_original_grm_trait_2dspl_rep1;
        my @varcomp_h_grm_trait_2dspl_rep1;
        my @fits_grm_trait_2dspl_rep1;
        my %seen_accession_names_2dspl_rep1;

        my $spatial_correct_2dspl_rep1_cmd = 'R -e "library(sommer); library(data.table); library(reshape2);
        mat <- data.frame(fread(\''.$stats_tempfile.'\', header=TRUE, sep=\',\'));
        geno_mat_3col <- data.frame(fread(\''.$grm_file.'\', header=FALSE, sep=\'\t\'));
        geno_mat <- acast(geno_mat_3col, V1~V2, value.var=\'V3\');
        geno_mat[is.na(geno_mat)] <- 0;
        ...
        "';
        print STDERR Dumper $spatial_correct_2dspl_rep1_cmd;
        my $spatial_correct_2dspl_rep1_status = system($spatial_correct_2dspl_rep1_cmd);
            ...
            while (my $row = <$fh>) {
                ...
                foreach my $encoded_trait (@header_cols) {
                    if ($encoded_trait eq $trait_name_encoded_string) {
                        ...
                        my $value = $columns[$col_counter+1];
                        if (defined $value && $value ne '') {
                            $result_blup_data_s_rep1->{$stock_name}->{$trait} = $value;
                            $seen_accession_names_2dspl_rep1{$stock_name}++;

                            if ($value < $genetic_effect_min_s_rep1) {
                                $genetic_effect_min_s_rep1 = $value;
                            }
                            elsif ($value >= $genetic_effect_max_s_rep1) {
                                $genetic_effect_max_s_rep1 = $value;
                            }

                            $genetic_effect_sum_s_rep1 += abs($value);
                            $genetic_effect_sum_square_s_rep1 = $genetic_effect_sum_square_s_rep1 + $value*$value;
                        }
                    }
                    $col_counter++;
                }
            ...
            while (my $row_2dspl = <$fh_2dspl>) {
                ...
                foreach my $encoded_trait (@header_cols_2dspl) {
                    if ($encoded_trait eq $trait_name_encoded_string) {
                        ...
                        my $value = $columns[$col_counter+1];
                        if (defined $value && $value ne '') {
                            $result_blup_spatial_data_s_rep1->{$plot_name}->{$trait} = $value;

                            if ($value < $env_effect_min_s_rep1) {
                                $env_effect_min_s_rep1 = $value;
                            }
                            elsif ($value >= $env_effect_max_s_rep1) {
                                $env_effect_max_s_rep1 = $value;
                            }

                            $env_effect_sum_s_rep1 += abs($value);
                            $env_effect_sum_square_s_rep1 = $env_effect_sum_square_s_rep1 + $value*$value;
                        }
                    }
                    $col_counter++;
                }
        ...
        open(my $fh_residual, '<', $stats_out_tempfile_residual) or die "Could not open file '$stats_out_tempfile_residual' $!";
            ...
            while (my $row = <$fh_residual>) {
                ...
                my $stock_name = $plot_id_map{$stock_id};
                if (defined $residual && $residual ne '') {
                    $result_residual_data_s_rep1->{$stock_name}->{$trait_name} = $residual;
                    $residual_sum_s_rep1 += abs($residual);
                    $residual_sum_square_s_rep1 = $residual_sum_square_s_rep1 + $residual*$residual;
                }
                if (defined $fitted && $fitted ne '') {
                    $result_fitted_data_s_rep1->{$stock_name}->{$trait_name} = $fitted;
                }
                $model_sum_square_residual_s_rep1 = $model_sum_square_residual_s_rep1 + $residual*$residual;
            }
        close($fh_residual);
        ...
        open(my $fh_varcomp, '<', $stats_out_tempfile_varcomp) or die "Could not open file '$stats_out_tempfile_varcomp' $!";
            ...
            while (my $row = <$fh_varcomp>) {
                ...
                push @varcomp_original_grm_trait_2dspl_rep1, \@columns;
            }
        close($fh_varcomp);
        ...
        open(my $fh_varcomp_h, '<', $stats_out_tempfile_vpredict) or die "Could not open file '$stats_out_tempfile_vpredict' $!";
            ...
            while (my $row = <$fh_varcomp_h>) {
                ...
                push @varcomp_h_grm_trait_2dspl_rep1, \@columns;
            }
        close($fh_varcomp_h);
        ...
        open(my $fh_fits, '<', $stats_out_tempfile_fits) or die "Could not open file '$stats_out_tempfile_fits' $!";
            ...
            while (my $row = <$fh_fits>) {
                ...
                push @fits_grm_trait_2dspl_rep1, \@columns;
            }
        close($fh_fits);

        my $result_blup_data_s_rep2;
        my $genetic_effect_max_s_rep2 = -1000000000;
        my $genetic_effect_min_s_rep2 = 10000000000;
        my $genetic_effect_sum_square_s_rep2 = 0;
        my $genetic_effect_sum_s_rep2 = 0;
        my $result_blup_spatial_data_s_rep2;
        my $env_effect_min_s_rep2 = 100000000;
        my $env_effect_max_s_rep2 = -100000000;
        my $env_effect_sum_s_rep2 = 0;
        my $env_effect_sum_square_s_rep2 = 0;
        my $result_residual_data_s_rep2;
        my $result_fitted_data_s_rep2;
        my $residual_sum_s_rep2 = 0;
        my $residual_sum_square_s_rep2 = 0;
        my $model_sum_square_residual_s_rep2 = 0;
        my @varcomp_original_grm_trait_2dspl_rep2;
        my @varcomp_h_grm_trait_2dspl_rep2;
        my @fits_grm_trait_2dspl_rep2;
        my %seen_accession_names_2dspl_rep2;

        my $spatial_correct_2dspl_rep2_cmd = 'R -e "library(sommer); library(data.table); library(reshape2);
        mat <- data.frame(fread(\''.$stats_tempfile.'\', header=TRUE, sep=\',\'));
        geno_mat_3col <- data.frame(fread(\''.$grm_file.'\', header=FALSE, sep=\'\t\'));
        geno_mat <- acast(geno_mat_3col, V1~V2, value.var=\'V3\');
        geno_mat[is.na(geno_mat)] <- 0;
        ...
        "';
        print STDERR Dumper $spatial_correct_2dspl_rep2_cmd;
        my $spatial_correct_2dspl_rep2_status = system($spatial_correct_2dspl_rep2_cmd);
            ...
            while (my $row = <$fh>) {
                ...
                foreach my $encoded_trait (@header_cols) {
                    if ($encoded_trait eq $trait_name_encoded_string) {
                        ...
                        my $value = $columns[$col_counter+1];
                        if (defined $value && $value ne '') {
                            $result_blup_data_s_rep2->{$stock_name}->{$trait} = $value;
                            $seen_accession_names_2dspl_rep2{$stock_name}++;

                            if ($value < $genetic_effect_min_s_rep2) {
                                $genetic_effect_min_s_rep2 = $value;
                            }
                            elsif ($value >= $genetic_effect_max_s_rep2) {
                                $genetic_effect_max_s_rep2 = $value;
                            }

                            $genetic_effect_sum_s_rep2 += abs($value);
                            $genetic_effect_sum_square_s_rep2 = $genetic_effect_sum_square_s_rep2 + $value*$value;
                        }
                    }
                    $col_counter++;
                }
        ...
        open($fh_2dspl, '<', $stats_out_tempfile_2dspl) or die "Could not open file '$stats_out_tempfile_2dspl' $!";
            print STDERR "Opened $stats_out_tempfile_2dspl\n";
            $header_2dspl = <$fh_2dspl>;
            @header_cols_2dspl = ();
            if ($csv->parse($header_2dspl)) {
                @header_cols_2dspl = $csv->fields();
            }
            ...
            while (my $row_2dspl = <$fh_2dspl>) {
                ...
                foreach my $encoded_trait (@header_cols_2dspl) {
                    if ($encoded_trait eq $trait_name_encoded_string) {
                        ...
                        my $value = $columns[$col_counter+1];
                        if (defined $value && $value ne '') {
                            $result_blup_spatial_data_s_rep2->{$plot_name}->{$trait} = $value;

                            if ($value < $env_effect_min_s_rep2) {
                                $env_effect_min_s_rep2 = $value;
                            }
                            elsif ($value >= $env_effect_max_s_rep2) {
                                $env_effect_max_s_rep2 = $value;
                            }

                            $env_effect_sum_s_rep2 += abs($value);
                            $env_effect_sum_square_s_rep2 = $env_effect_sum_square_s_rep2 + $value*$value;
                        }
                    }
                    $col_counter++;
                }
        ...
        open($fh_residual, '<', $stats_out_tempfile_residual) or die "Could not open file '$stats_out_tempfile_residual' $!";
            ...
            while (my $row = <$fh_residual>) {
                ...
                my $stock_name = $plot_id_map{$stock_id};
                if (defined $residual && $residual ne '') {
                    $result_residual_data_s_rep2->{$stock_name}->{$trait_name} = $residual;
                    $residual_sum_s_rep2 += abs($residual);
                    $residual_sum_square_s_rep2 = $residual_sum_square_s_rep2 + $residual*$residual;
                }
                if (defined $fitted && $fitted ne '') {
                    $result_fitted_data_s_rep2->{$stock_name}->{$trait_name} = $fitted;
                }
                $model_sum_square_residual_s_rep2 = $model_sum_square_residual_s_rep2 + $residual*$residual;
            }
        close($fh_residual);
        ...
        open($fh_varcomp, '<', $stats_out_tempfile_varcomp) or die "Could not open file '$stats_out_tempfile_varcomp' $!";
            ...
            while (my $row = <$fh_varcomp>) {
                ...
                push @varcomp_original_grm_trait_2dspl_rep2, \@columns;
            }
        close($fh_varcomp);
        ...
        open($fh_varcomp_h, '<', $stats_out_tempfile_vpredict) or die "Could not open file '$stats_out_tempfile_vpredict' $!";
            ...
            while (my $row = <$fh_varcomp_h>) {
                ...
                push @varcomp_h_grm_trait_2dspl_rep2, \@columns;
            }
        close($fh_varcomp_h);
        ...
        open($fh_fits, '<', $stats_out_tempfile_fits) or die "Could not open file '$stats_out_tempfile_fits' $!";
            ...
            while (my $row = <$fh_fits>) {
                ...
                push @fits_grm_trait_2dspl_rep2, \@columns;
            }
        close($fh_fits);

        my @seen_accession_names_2dspl_rep1_rep2;
        foreach my $a (sort keys %seen_accession_names_2dspl_rep1) {
            if (exists($seen_accession_names_2dspl_rep2{$a})) {
                push @seen_accession_names_2dspl_rep1_rep2, $a;
            }
        }

        open(my $F_gcorr, ">", $ggcorr_tempfile) || die "Can't open file ".$ggcorr_tempfile;
            print $F_gcorr "accession_name,g_rep1,g_rep2\n";

            foreach my $stock_name (@seen_accession_names_2dspl_rep1_rep2) {
                foreach my $trait_name (@sorted_trait_names) {
                    my $val1 = $result_blup_data_s_rep1->{$stock_name}->{$trait_name} || 'NA';
                    my $val2 = $result_blup_data_s_rep2->{$stock_name}->{$trait_name} || 'NA';
                    print $F_gcorr "$stock_name,$val1,$val2\n";
                }
            }
        close($F_gcorr);

        my $grm_gcor_cmd = 'R -e "library(data.table);
        mat <- data.frame(fread(\''.$ggcorr_tempfile.'\', header=TRUE, sep=\',\'));
        g_corr <- 0;
        try (g_corr <- cor(mat\$g_rep1, mat\$g_rep2, use = \'complete.obs\'));
        write.table(data.frame(gcorr = c(g_corr) ), file=\''.$stats_out_tempfile_gcor.'\', row.names=FALSE, col.names=TRUE, sep=\',\');
        "';
        print STDERR Dumper $grm_gcor_cmd;
        my $grm_gcor_cmd_status = system($grm_gcor_cmd);
    ...
    while (my ($model_id, $model_name, $model_description, $basename, $filename, $file_id, $filetype, $model_type_id, $experiment_type_id, $property_type_id, $property_value) = $h->fetchrow_array()) {
        ...
        foreach my $p (@seen_plots) {
            ...
            foreach my $t (@sorted_trait_names) {
                ...
                foreach my $time (@sorted_seen_times_p) {
                    ...
                    push @{$plot_fixed_effects_3{$p}->{$fixed_eff_counter}}, $fixed_val;

                    if ($time_point_counter >= $number_time_points_1third) {
                        $fixed_eff_counter++;
                        $time_point_counter = 0;
                    }
        ...
        if ($result_type eq 'fullcorr') {
            ...
            open(my $F29, ">", $analytics_protocol_data_tempfile29) || die "Can't open file ".$analytics_protocol_data_tempfile29;
                print $F29 "plot_name,fixed_effect_all_cont,fixed_effect_all,fixed_effect_1,fixed_effect_2,fixed_effect_3_1,fixed_effect_3_2,fixed_effect_3_3,fixed_effect_max,fixed_effect_min\n";
                foreach my $p (@seen_plots) {
                    ...
                    my $fixed_effect_3_1 = $plot_averaged_fixed_effects_3{$p}->{'1'};
                    my $fixed_effect_3_2 = $plot_averaged_fixed_effects_3{$p}->{'2'};
                    my $fixed_effect_3_3 = $plot_averaged_fixed_effects_3{$p}->{'3'};
                    print $F29 "$p,$fixed_effect_cont,$fixed_effect_all,$fixed_effect_1,$fixed_effect_2,$fixed_effect_3_1,$fixed_effect_3_2,$fixed_effect_3_3,$fixed_effect_max,$fixed_effect_min\n";
                }
            close($F29);
            ...
            my $result_blup_data_s_favg_rep1;
            my $genetic_effect_min_s_favg_rep1 = 10000000000;
            my $genetic_effect_max_s_favg_rep1 = -10000000000;
            my $genetic_effect_sum_s_favg_rep1 = 0;
            my $genetic_effect_sum_square_s_favg_rep1 = 0;
            my $result_residual_data_s_favg_rep1;
            my $residual_sum_s_favg_rep1 = 0;
            my $residual_sum_square_s_favg_rep1 = 0;
            my $result_fitted_data_s_favg_rep1;
            my $model_sum_square_residual_s_favg_rep1 = 0;
            my @varcomp_original_grm_fixed_effect_favg_rep1;
            my @varcomp_h_grm_fixed_effect_favg_rep1;
            my @fits_grm_fixed_effect_favg_rep1;
            my %seen_accession_names_favg_rep1;

            my $grm_no_prm_fixed_effect_rep1_cmd = 'R -e "library(sommer); library(data.table); library(reshape2); library(ggplot2); library(GGally);
            mat <- data.frame(fread(\''.$stats_tempfile.'\', header=TRUE, sep=\',\'));
            mat_fixed <- data.frame(fread(\''.$analytics_protocol_data_tempfile29.'\', header=TRUE, sep=\',\'));
            geno_mat_3col <- data.frame(fread(\''.$grm_file.'\', header=FALSE, sep=\'\t\'));
            geno_mat <- acast(geno_mat_3col, V1~V2, value.var=\'V3\');
            ...
            "';
            print STDERR Dumper $grm_no_prm_fixed_effect_rep1_cmd;
            my $grm_no_prm_fixed_effect_rep1_cmd_status = system($grm_no_prm_fixed_effect_rep1_cmd);
                ...
                while (my $row = <$fh>) {
                    ...
                    foreach my $encoded_trait (@header_cols_no_prm) {
                        if ($encoded_trait eq $trait_name_encoded_string) {
                            ...
                            my $value = $columns[$col_counter+1];
                            if (defined $value && $value ne '') {
                                $result_blup_data_s_favg_rep1->{$stock_name}->{$trait} = $value;
                                $seen_accession_names_favg_rep1{$stock_name}++;

                                if ($value < $genetic_effect_min_s_favg_rep1) {
                                    $genetic_effect_min_s_favg_rep1 = $value;
                                }
                                elsif ($value >= $genetic_effect_max_s_favg_rep1) {
                                    $genetic_effect_max_s_favg_rep1 = $value;
                                }

                                $genetic_effect_sum_s_favg_rep1 += abs($value);
                                $genetic_effect_sum_square_s_favg_rep1 = $genetic_effect_sum_square_s_favg_rep1 + $value*$value;
                            }
                        }
                        $col_counter++;
                    }
            ...
            open($fh_residual, '<', $stats_out_tempfile_residual) or die "Could not open file '$stats_out_tempfile_residual' $!";
                ...
                while (my $row = <$fh_residual>) {
                    ...
                    my $stock_name = $plot_id_map{$stock_id};
                    if (defined $residual && $residual ne '') {
                        $result_residual_data_s_favg_rep1->{$stock_name}->{$trait_name} = $residual;
                        $residual_sum_s_favg_rep1 += abs($residual);
                        $residual_sum_square_s_favg_rep1 = $residual_sum_square_s_favg_rep1 + $residual*$residual;
                    }
                    if (defined $fitted && $fitted ne '') {
                        $result_fitted_data_s_favg_rep1->{$stock_name}->{$trait_name} = $fitted;
                    }
                    $model_sum_square_residual_s_favg_rep1 = $model_sum_square_residual_s_favg_rep1 + $residual*$residual;
                }
            close($fh_residual);
            ...
            open($fh_varcomp, '<', $stats_out_tempfile_varcomp) or die "Could not open file '$stats_out_tempfile_varcomp' $!";
                ...
                while (my $row = <$fh_varcomp>) {
                    ...
                    push @varcomp_original_grm_fixed_effect_favg_rep1, \@columns;
                }
            close($fh_varcomp);
            ...
            open($fh_varcomp_h, '<', $stats_out_tempfile_vpredict) or die "Could not open file '$stats_out_tempfile_vpredict' $!";
                ...
                while (my $row = <$fh_varcomp_h>) {
                    ...
                    push @varcomp_h_grm_fixed_effect_favg_rep1, \@columns;
                }
            close($fh_varcomp_h);
            ...
            open($fh_fits, '<', $stats_out_tempfile_fits) or die "Could not open file '$stats_out_tempfile_fits' $!";
                ...
                while (my $row = <$fh_fits>) {
                    ...
                    push @fits_grm_fixed_effect_favg_rep1, \@columns;
                }
            close($fh_fits);

            my $result_blup_data_s_favg_rep2;
            my $genetic_effect_min_s_favg_rep2 = 10000000000;
            my $genetic_effect_max_s_favg_rep2 = -10000000000;
            my $genetic_effect_sum_s_favg_rep2 = 0;
            my $genetic_effect_sum_square_s_favg_rep2 = 0;
            my $result_residual_data_s_favg_rep2;
            my $residual_sum_s_favg_rep2 = 0;
            my $residual_sum_square_s_favg_rep2 = 0;
            my $result_fitted_data_s_favg_rep2;
            my $model_sum_square_residual_s_favg_rep2 = 0;
            my @varcomp_original_grm_fixed_effect_favg_rep2;
            my @varcomp_h_grm_fixed_effect_favg_rep2;
            my @fits_grm_fixed_effect_favg_rep2;
            my %seen_accession_names_favg_rep2;

            my $grm_no_prm_fixed_effect_rep2_cmd = 'R -e "library(sommer); library(data.table); library(reshape2); library(ggplot2); library(GGally);
            mat <- data.frame(fread(\''.$stats_tempfile.'\', header=TRUE, sep=\',\'));
            mat_fixed <- data.frame(fread(\''.$analytics_protocol_data_tempfile29.'\', header=TRUE, sep=\',\'));
            geno_mat_3col <- data.frame(fread(\''.$grm_file.'\', header=FALSE, sep=\'\t\'));
            geno_mat <- acast(geno_mat_3col, V1~V2, value.var=\'V3\');
            ...
            "';
            print STDERR Dumper $grm_no_prm_fixed_effect_rep2_cmd;
            my $grm_no_prm_fixed_effect_rep2_cmd_status = system($grm_no_prm_fixed_effect_rep2_cmd);
                ...
                while (my $row = <$fh>) {
                    ...
                    foreach my $encoded_trait (@header_cols_no_prm) {
                        if ($encoded_trait eq $trait_name_encoded_string) {
                            ...
                            my $value = $columns[$col_counter+1];
                            if (defined $value && $value ne '') {
                                $result_blup_data_s_favg_rep2->{$stock_name}->{$trait} = $value;
                                $seen_accession_names_favg_rep2{$stock_name}++;

                                if ($value < $genetic_effect_min_s_favg_rep2) {
                                    $genetic_effect_min_s_favg_rep2 = $value;
                                }
                                elsif ($value >= $genetic_effect_max_s_favg_rep2) {
                                    $genetic_effect_max_s_favg_rep2 = $value;
                                }

                                $genetic_effect_sum_s_favg_rep2 += abs($value);
                                $genetic_effect_sum_square_s_favg_rep2 = $genetic_effect_sum_square_s_favg_rep2 + $value*$value;
                            }
                        }
                        $col_counter++;
                    }
            ...
            open($fh_residual, '<', $stats_out_tempfile_residual) or die "Could not open file '$stats_out_tempfile_residual' $!";
                ...
                while (my $row = <$fh_residual>) {
                    ...
                    my $stock_name = $plot_id_map{$stock_id};
                    if (defined $residual && $residual ne '') {
                        $result_residual_data_s_favg_rep2->{$stock_name}->{$trait_name} = $residual;
                        $residual_sum_s_favg_rep2 += abs($residual);
                        $residual_sum_square_s_favg_rep2 = $residual_sum_square_s_favg_rep2 + $residual*$residual;
                    }
                    if (defined $fitted && $fitted ne '') {
                        $result_fitted_data_s_favg_rep2->{$stock_name}->{$trait_name} = $fitted;
                    }
                    $model_sum_square_residual_s_favg_rep2 = $model_sum_square_residual_s_favg_rep2 + $residual*$residual;
                }
            close($fh_residual);
            ...
            open($fh_varcomp, '<', $stats_out_tempfile_varcomp) or die "Could not open file '$stats_out_tempfile_varcomp' $!";
                ...
                while (my $row = <$fh_varcomp>) {
                    ...
                    push @varcomp_original_grm_fixed_effect_favg_rep2, \@columns;
                }
            close($fh_varcomp);
            ...
            open($fh_varcomp_h, '<', $stats_out_tempfile_vpredict) or die "Could not open file '$stats_out_tempfile_vpredict' $!";
                ...
                while (my $row = <$fh_varcomp_h>) {
                    ...
                    push @varcomp_h_grm_fixed_effect_favg_rep2, \@columns;
                }
            close($fh_varcomp_h);
            ...
            open($fh_fits, '<', $stats_out_tempfile_fits) or die "Could not open file '$stats_out_tempfile_fits' $!";
                ...
                while (my $row = <$fh_fits>) {
                    ...
                    push @fits_grm_fixed_effect_favg_rep2, \@columns;
                }
            close($fh_fits);

            my @seen_accession_names_favg_rep1_rep2;
            foreach my $a (sort keys %seen_accession_names_favg_rep1) {
                if (exists($seen_accession_names_favg_rep2{$a})) {
                    push @seen_accession_names_favg_rep1_rep2, $a;
                }
            }

            open(my $F_avg_gcorr, ">", $analytics_protocol_data_tempfile_favg_gcorr) || die "Can't open file ".$analytics_protocol_data_tempfile_favg_gcorr;
                print $F_avg_gcorr "accession_name,g_rep1,g_rep2\n";

                foreach my $stock_name (@seen_accession_names_favg_rep1_rep2) {
                    foreach my $trait_name (@sorted_trait_names) {
                        my $val1 = $result_blup_data_s_favg_rep1->{$stock_name}->{$trait_name} || 'NA';
                        my $val2 = $result_blup_data_s_favg_rep2->{$stock_name}->{$trait_name} || 'NA';
                        print $F_avg_gcorr "$stock_name,$val1,$val2\n";
                    }
                }
            close($F_avg_gcorr);
            ...
            my $result_blup_data_s_f2_rep1;
            my $genetic_effect_min_s_f2_rep1 = 10000000000;
            my $genetic_effect_max_s_f2_rep1 = -10000000000;
            my $genetic_effect_sum_s_f2_rep1 = 0;
            my $genetic_effect_sum_square_s_f2_rep1 = 0;
            my $result_residual_data_s_f2_rep1;
            my $residual_sum_s_f2_rep1 = 0;
            my $residual_sum_square_s_f2_rep1 = 0;
            my $result_fitted_data_s_f2_rep1;
            my $model_sum_square_residual_s_f2_rep1 = 0;
            my @varcomp_original_grm_fixed_effect_f2_rep1;
            my @varcomp_h_grm_fixed_effect_f2_rep1;
            my @fits_grm_fixed_effect_f2_rep1;
            my %seen_accession_names_f2_rep1;

            my $grm_no_prm_fixed_effects_rep1_cmd = 'R -e "library(sommer); library(data.table); library(reshape2); library(ggplot2); library(GGally);
            mat <- data.frame(fread(\''.$stats_tempfile.'\', header=TRUE, sep=\',\'));
            mat_fixed <- data.frame(fread(\''.$analytics_protocol_data_tempfile29.'\', header=TRUE, sep=\',\'));
            geno_mat_3col <- data.frame(fread(\''.$grm_file.'\', header=FALSE, sep=\'\t\'));
            geno_mat <- acast(geno_mat_3col, V1~V2, value.var=\'V3\');
            ...
            "';
            print STDERR Dumper $grm_no_prm_fixed_effects_rep1_cmd;
            my $grm_no_prm_fixed_effects_rep1_cmd_status = system($grm_no_prm_fixed_effects_rep1_cmd);
                ...
                while (my $row = <$fh>) {
                    ...
                    foreach my $encoded_trait (@header_cols_no_prm) {
                        if ($encoded_trait eq $trait_name_encoded_string) {
                            ...
                            my $value = $columns[$col_counter+1];
                            if (defined $value && $value ne '') {
                                $result_blup_data_s_f2_rep1->{$stock_name}->{$trait} = $value;
                                $seen_accession_names_f2_rep1{$stock_name}++;

                                if ($value < $genetic_effect_min_s_f2_rep1) {
                                    $genetic_effect_min_s_f2_rep1 = $value;
                                }
                                elsif ($value >= $genetic_effect_max_s_f2_rep1) {
                                    $genetic_effect_max_s_f2_rep1 = $value;
                                }

                                $genetic_effect_sum_s_f2_rep1 += abs($value);
                                $genetic_effect_sum_square_s_f2_rep1 = $genetic_effect_sum_square_s_f2_rep1 + $value*$value;
                            }
                        }
                        $col_counter++;
                    }
            ...
            open($fh_residual, '<', $stats_out_tempfile_residual) or die "Could not open file '$stats_out_tempfile_residual' $!";
                ...
                while (my $row = <$fh_residual>) {
                    ...
                    my $stock_name = $plot_id_map{$stock_id};
                    if (defined $residual && $residual ne '') {
                        $result_residual_data_s_f2_rep1->{$stock_name}->{$trait_name} = $residual;
                        $residual_sum_s_f2_rep1 += abs($residual);
                        $residual_sum_square_s_f2_rep1 = $residual_sum_square_s_f2_rep1 + $residual*$residual;
                    }
                    if (defined $fitted && $fitted ne '') {
                        $result_fitted_data_s_f2_rep1->{$stock_name}->{$trait_name} = $fitted;
                    }
                    $model_sum_square_residual_s_f2_rep1 = $model_sum_square_residual_s_f2_rep1 + $residual*$residual;
                }
            close($fh_residual);
            ...
            open($fh_varcomp, '<', $stats_out_tempfile_varcomp) or die "Could not open file '$stats_out_tempfile_varcomp' $!";
                ...
                while (my $row = <$fh_varcomp>) {
                    ...
                    push @varcomp_original_grm_fixed_effect_f2_rep1, \@columns;
                }
            close($fh_varcomp);
            ...
            open($fh_varcomp_h, '<', $stats_out_tempfile_vpredict) or die "Could not open file '$stats_out_tempfile_vpredict' $!";
                ...
                while (my $row = <$fh_varcomp_h>) {
                    ...
                    push @varcomp_h_grm_fixed_effect_f2_rep1, \@columns;
                }
            close($fh_varcomp_h);
            ...
            open($fh_fits, '<', $stats_out_tempfile_fits) or die "Could not open file '$stats_out_tempfile_fits' $!";
                ...
                while (my $row = <$fh_fits>) {
                    ...
                    push @fits_grm_fixed_effect_f2_rep1, \@columns;
                }
            close($fh_fits);

            my $result_blup_data_s_f2_rep2;
            my $genetic_effect_min_s_f2_rep2 = 10000000000;
            my $genetic_effect_max_s_f2_rep2 = -10000000000;
            my $genetic_effect_sum_s_f2_rep2 = 0;
            my $genetic_effect_sum_square_s_f2_rep2 = 0;
            my $result_residual_data_s_f2_rep2;
            my $residual_sum_s_f2_rep2 = 0;
            my $residual_sum_square_s_f2_rep2 = 0;
            my $result_fitted_data_s_f2_rep2;
            my $model_sum_square_residual_s_f2_rep2 = 0;
            my @varcomp_original_grm_fixed_effect_f2_rep2;
            my @varcomp_h_grm_fixed_effect_f2_rep2;
            my @fits_grm_fixed_effect_f2_rep2;
            my %seen_accession_names_f2_rep2;

            my $grm_no_prm_fixed_effects_rep2_cmd = 'R -e "library(sommer); library(data.table); library(reshape2); library(ggplot2); library(GGally);
            mat <- data.frame(fread(\''.$stats_tempfile.'\', header=TRUE, sep=\',\'));
            mat_fixed <- data.frame(fread(\''.$analytics_protocol_data_tempfile29.'\', header=TRUE, sep=\',\'));
            geno_mat_3col <- data.frame(fread(\''.$grm_file.'\', header=FALSE, sep=\'\t\'));
            geno_mat <- acast(geno_mat_3col, V1~V2, value.var=\'V3\');
            ...
            "';
            print STDERR Dumper $grm_no_prm_fixed_effects_rep2_cmd;
            my $grm_no_prm_fixed_effects_rep2_cmd_status = system($grm_no_prm_fixed_effects_rep2_cmd);
                ...
                while (my $row = <$fh>) {
                    ...
                    foreach my $encoded_trait (@header_cols_no_prm) {
                        if ($encoded_trait eq $trait_name_encoded_string) {
                            ...
                            my $value = $columns[$col_counter+1];
                            if (defined $value && $value ne '') {
                                $result_blup_data_s_f2_rep2->{$stock_name}->{$trait} = $value;
                                $seen_accession_names_f2_rep2{$stock_name}++;

                                if ($value < $genetic_effect_min_s_f2_rep2) {
                                    $genetic_effect_min_s_f2_rep2 = $value;
                                }
                                elsif ($value >= $genetic_effect_max_s_f2_rep2) {
                                    $genetic_effect_max_s_f2_rep2 = $value;
                                }

                                $genetic_effect_sum_s_f2_rep2 += abs($value);
                                $genetic_effect_sum_square_s_f2_rep2 = $genetic_effect_sum_square_s_f2_rep2 + $value*$value;
                            }
                        }
                        $col_counter++;
                    }
            ...
            open($fh_residual, '<', $stats_out_tempfile_residual) or die "Could not open file '$stats_out_tempfile_residual' $!";
                ...
                while (my $row = <$fh_residual>) {
                    ...
                    my $stock_name = $plot_id_map{$stock_id};
                    if (defined $residual && $residual ne '') {
                        $result_residual_data_s_f2_rep2->{$stock_name}->{$trait_name} = $residual;
                        $residual_sum_s_f2_rep2 += abs($residual);
                        $residual_sum_square_s_f2_rep2 = $residual_sum_square_s_f2_rep2 + $residual*$residual;
                    }
                    if (defined $fitted && $fitted ne '') {
                        $result_fitted_data_s_f2_rep2->{$stock_name}->{$trait_name} = $fitted;
                    }
                    $model_sum_square_residual_s_f2_rep2 = $model_sum_square_residual_s_f2_rep2 + $residual*$residual;
                }
            close($fh_residual);
            ...
            open($fh_varcomp, '<', $stats_out_tempfile_varcomp) or die "Could not open file '$stats_out_tempfile_varcomp' $!";
                ...
                while (my $row = <$fh_varcomp>) {
                    ...
                    push @varcomp_original_grm_fixed_effect_f2_rep2, \@columns;
                }
            close($fh_varcomp);
            ...
            open($fh_varcomp_h, '<', $stats_out_tempfile_vpredict) or die "Could not open file '$stats_out_tempfile_vpredict' $!";
                ...
                while (my $row = <$fh_varcomp_h>) {
                    ...
                    push @varcomp_h_grm_fixed_effect_f2_rep2, \@columns;
                }
            close($fh_varcomp_h);
            ...
            open($fh_fits, '<', $stats_out_tempfile_fits) or die "Could not open file '$stats_out_tempfile_fits' $!";
                ...
                while (my $row = <$fh_fits>) {
                    ...
                    push @fits_grm_fixed_effect_f2_rep2, \@columns;
                }
            close($fh_fits);

            my @seen_accession_names_f2_rep1_rep2;
            foreach my $a (sort keys %seen_accession_names_f2_rep1) {
                if (exists($seen_accession_names_f2_rep2{$a})) {
                    push @seen_accession_names_f2_rep1_rep2, $a;
                }
            }

            open(my $F_2_gcorr, ">", $analytics_protocol_data_tempfile_f2_gcorr) || die "Can't open file ".$analytics_protocol_data_tempfile_f2_gcorr;
                print $F_2_gcorr "accession_name,g_rep1,g_rep2\n";

                foreach my $stock_name (@seen_accession_names_f2_rep1_rep2) {
                    foreach my $trait_name (@sorted_trait_names) {
                        my $val1 = $result_blup_data_s_f2_rep1->{$stock_name}->{$trait_name} || 'NA';
                        my $val2 = $result_blup_data_s_f2_rep2->{$stock_name}->{$trait_name} || 'NA';
                        print $F_2_gcorr "$stock_name,$val1,$val2\n";
                    }
                }
            close($F_2_gcorr);
            ...
            my $result_blup_data_s_f3_rep1;
            my $genetic_effect_min_s_f3_rep1 = 10000000000;
            my $genetic_effect_max_s_f3_rep1 = -10000000000;
            my $genetic_effect_sum_s_f3_rep1 = 0;
            my $genetic_effect_sum_square_s_f3_rep1 = 0;
            my $result_residual_data_s_f3_rep1;
            my $residual_sum_s_f3_rep1 = 0;
            my $residual_sum_square_s_f3_rep1 = 0;
            my $result_fitted_data_s_f3_rep1;
            my $model_sum_square_residual_s_f3_rep1 = 0;
            my @varcomp_original_grm_fixed_effect_f3_rep1;
            my @varcomp_h_grm_fixed_effect_f3_rep1;
            my @fits_grm_fixed_effect_f3_rep1;
            my %seen_accession_names_f3_rep1;

            my $grm_no_prm_fixed_effects_3_rep1_cmd = 'R -e "library(sommer); library(data.table); library(reshape2); library(ggplot2); library(GGally);
            mat <- data.frame(fread(\''.$stats_tempfile.'\', header=TRUE, sep=\',\'));
            mat_fixed <- data.frame(fread(\''.$analytics_protocol_data_tempfile29.'\', header=TRUE, sep=\',\'));
            geno_mat_3col <- data.frame(fread(\''.$grm_file.'\', header=FALSE, sep=\'\t\'));
            geno_mat <- acast(geno_mat_3col, V1~V2, value.var=\'V3\');
            ...
            "';
            print STDERR Dumper $grm_no_prm_fixed_effects_3_rep1_cmd;
            my $grm_no_prm_fixed_effects_3_rep1_cmd_status = system($grm_no_prm_fixed_effects_3_rep1_cmd);
                ...
                while (my $row = <$fh>) {
                    ...
                    foreach my $encoded_trait (@header_cols_no_prm) {
                        if ($encoded_trait eq $trait_name_encoded_string) {
                            ...
                            my $value = $columns[$col_counter+1];
                            if (defined $value && $value ne '') {
                                $result_blup_data_s_f3_rep1->{$stock_name}->{$trait} = $value;
                                $seen_accession_names_f3_rep1{$stock_name}++;

                                if ($value < $genetic_effect_min_s_f3_rep1) {
                                    $genetic_effect_min_s_f3_rep1 = $value;
                                }
                                elsif ($value >= $genetic_effect_max_s_f3_rep1) {
                                    $genetic_effect_max_s_f3_rep1 = $value;
                                }

                                $genetic_effect_sum_s_f3_rep1 += abs($value);
                                $genetic_effect_sum_square_s_f3_rep1 = $genetic_effect_sum_square_s_f3_rep1 + $value*$value;
                            }
                        }
                        $col_counter++;
                    }
            ...
            open($fh_residual, '<', $stats_out_tempfile_residual) or die "Could not open file '$stats_out_tempfile_residual' $!";
                ...
                while (my $row = <$fh_residual>) {
                    ...
                    my $stock_name = $plot_id_map{$stock_id};
                    if (defined $residual && $residual ne '') {
                        $result_residual_data_s_f3_rep1->{$stock_name}->{$trait_name} = $residual;
                        $residual_sum_s_f3_rep1 += abs($residual);
                        $residual_sum_square_s_f3_rep1 = $residual_sum_square_s_f3_rep1 + $residual*$residual;
                    }
                    if (defined $fitted && $fitted ne '') {
                        $result_fitted_data_s_f3_rep1->{$stock_name}->{$trait_name} = $fitted;
                    }
                    $model_sum_square_residual_s_f3_rep1 = $model_sum_square_residual_s_f3_rep1 + $residual*$residual;
                }
            close($fh_residual);
            ...
            open($fh_varcomp, '<', $stats_out_tempfile_varcomp) or die "Could not open file '$stats_out_tempfile_varcomp' $!";
                ...
                while (my $row = <$fh_varcomp>) {
                    ...
                    push @varcomp_original_grm_fixed_effect_f3_rep1, \@columns;
                }
            close($fh_varcomp);
            ...
            open($fh_varcomp_h, '<', $stats_out_tempfile_vpredict) or die "Could not open file '$stats_out_tempfile_vpredict' $!";
                ...
                while (my $row = <$fh_varcomp_h>) {
                    ...
                    push @varcomp_h_grm_fixed_effect_f3_rep1, \@columns;
                }
            close($fh_varcomp_h);
            ...
            open($fh_fits, '<', $stats_out_tempfile_fits) or die "Could not open file '$stats_out_tempfile_fits' $!";
                ...
                while (my $row = <$fh_fits>) {
                    ...
                    push @fits_grm_fixed_effect_f3_rep1, \@columns;
                }
            close($fh_fits);

            my $result_blup_data_s_f3_rep2;
            my $genetic_effect_min_s_f3_rep2 = 10000000000;
            my $genetic_effect_max_s_f3_rep2 = -10000000000;
            my $genetic_effect_sum_s_f3_rep2 = 0;
            my $genetic_effect_sum_square_s_f3_rep2 = 0;
            my $result_residual_data_s_f3_rep2;
            my $residual_sum_s_f3_rep2 = 0;
            my $residual_sum_square_s_f3_rep2 = 0;
            my $result_fitted_data_s_f3_rep2;
            my $model_sum_square_residual_s_f3_rep2 = 0;
            my @varcomp_original_grm_fixed_effect_f3_rep2;
            my @varcomp_h_grm_fixed_effect_f3_rep2;
            my @fits_grm_fixed_effect_f3_rep2;
            my %seen_accession_names_f3_rep2;

            my $grm_no_prm_fixed_effects_3_rep2_cmd = 'R -e "library(sommer); library(data.table); library(reshape2); library(ggplot2); library(GGally);
            mat <- data.frame(fread(\''.$stats_tempfile.'\', header=TRUE, sep=\',\'));
            mat_fixed <- data.frame(fread(\''.$analytics_protocol_data_tempfile29.'\', header=TRUE, sep=\',\'));
            geno_mat_3col <- data.frame(fread(\''.$grm_file.'\', header=FALSE, sep=\'\t\'));
            geno_mat <- acast(geno_mat_3col, V1~V2, value.var=\'V3\');
            ...
            "';
            print STDERR Dumper $grm_no_prm_fixed_effects_3_rep2_cmd;
            my $grm_no_prm_fixed_effects_3_rep2_cmd_status = system($grm_no_prm_fixed_effects_3_rep2_cmd);
                ...
                while (my $row = <$fh>) {
                    ...
                    foreach my $encoded_trait (@header_cols_no_prm) {
                        if ($encoded_trait eq $trait_name_encoded_string) {
                            ...
                            my $value = $columns[$col_counter+1];
                            if (defined $value && $value ne '') {
                                $result_blup_data_s_f3_rep2->{$stock_name}->{$trait} = $value;
                                $seen_accession_names_f3_rep2{$stock_name}++;

                                if ($value < $genetic_effect_min_s_f3_rep2) {
                                    $genetic_effect_min_s_f3_rep2 = $value;
                                }
                                elsif ($value >= $genetic_effect_max_s_f3_rep2) {
                                    $genetic_effect_max_s_f3_rep2 = $value;
                                }

                                $genetic_effect_sum_s_f3_rep2 += abs($value);
                                $genetic_effect_sum_square_s_f3_rep2 = $genetic_effect_sum_square_s_f3_rep2 + $value*$value;
                            }
                        }
                        $col_counter++;
                    }
            ...
            open($fh_residual, '<', $stats_out_tempfile_residual) or die "Could not open file '$stats_out_tempfile_residual' $!";
                ...
                while (my $row = <$fh_residual>) {
                    ...
                    my $stock_name = $plot_id_map{$stock_id};
                    if (defined $residual && $residual ne '') {
                        $result_residual_data_s_f3_rep2->{$stock_name}->{$trait_name} = $residual;
                        $residual_sum_s_f3_rep2 += abs($residual);
                        $residual_sum_square_s_f3_rep2 = $residual_sum_square_s_f3_rep2 + $residual*$residual;
                    }
                    if (defined $fitted && $fitted ne '') {
                        $result_fitted_data_s_f3_rep2->{$stock_name}->{$trait_name} = $fitted;
                    }
                    $model_sum_square_residual_s_f3_rep2 = $model_sum_square_residual_s_f3_rep2 + $residual*$residual;
                }
            close($fh_residual);
            ...
            open($fh_varcomp, '<', $stats_out_tempfile_varcomp) or die "Could not open file '$stats_out_tempfile_varcomp' $!";
                ...
                while (my $row = <$fh_varcomp>) {
                    ...
                    push @varcomp_original_grm_fixed_effect_f3_rep2, \@columns;
                }
            close($fh_varcomp);
            ...
            open($fh_varcomp_h, '<', $stats_out_tempfile_vpredict) or die "Could not open file '$stats_out_tempfile_vpredict' $!";
                ...
                while (my $row = <$fh_varcomp_h>) {
                    ...
                    push @varcomp_h_grm_fixed_effect_f3_rep2, \@columns;
                }
            close($fh_varcomp_h);
            ...
            open($fh_fits, '<', $stats_out_tempfile_fits) or die "Could not open file '$stats_out_tempfile_fits' $!";
                ...
                while (my $row = <$fh_fits>) {
                    ...
                    push @fits_grm_fixed_effect_f3_rep2, \@columns;
                }
            close($fh_fits);

            my @seen_accession_names_f3_rep1_rep2;
            foreach my $a (sort keys %seen_accession_names_f3_rep1) {
                if (exists($seen_accession_names_f3_rep2{$a})) {
                    push @seen_accession_names_f3_rep1_rep2, $a;
                }
            }

            open(my $F_3_gcorr, ">", $analytics_protocol_data_tempfile_f3_gcorr) || die "Can't open file ".$analytics_protocol_data_tempfile_f3_gcorr;
                print $F_3_gcorr "accession_name,g_rep1,g_rep2\n";

                foreach my $stock_name (@seen_accession_names_f3_rep1_rep2) {
                    foreach my $trait_name (@sorted_trait_names) {
                        my $val1 = $result_blup_data_s_f3_rep1->{$stock_name}->{$trait_name} || 'NA';
                        my $val2 = $result_blup_data_s_f3_rep2->{$stock_name}->{$trait_name} || 'NA';
                        print $F_3_gcorr "$stock_name,$val1,$val2\n";
                    }
                }
            close($F_3_gcorr);
            ...
            my $result_blup_data_s_havg_rep1;
            my $genetic_effect_min_s_havg_rep1 = 10000000000;
            my $genetic_effect_max_s_havg_rep1 = -10000000000;
            my $genetic_effect_sum_s_havg_rep1 = 0;
            my $genetic_effect_sum_square_s_havg_rep1 = 0;
            my $result_residual_data_s_havg_rep1;
            my $residual_sum_s_havg_rep1 = 0;
            my $residual_sum_square_s_havg_rep1 = 0;
            my $result_fitted_data_s_havg_rep1;
            my $model_sum_square_residual_s_havg_rep1 = 0;
            my @varcomp_original_grm_fixed_effect_havg_rep1;
            my @varcomp_h_grm_fixed_effect_havg_rep1;
            my @fits_grm_fixed_effect_havg_rep1;
            my %seen_accession_names_havg_rep1;

            my $grm_no_prm_fixed_effects_havg_rep1_cmd = 'R -e "library(sommer); library(data.table); library(reshape2); library(ggplot2); library(GGally);
            mat <- data.frame(fread(\''.$stats_tempfile.'\', header=TRUE, sep=\',\'));
            mat_fixed <- data.frame(fread(\''.$analytics_protocol_data_tempfile29.'\', header=TRUE, sep=\',\'));
            geno_mat_3col <- data.frame(fread(\''.$grm_file.'\', header=FALSE, sep=\'\t\'));
            geno_mat <- acast(geno_mat_3col, V1~V2, value.var=\'V3\');
            ...
            "';
            print STDERR Dumper $grm_no_prm_fixed_effects_havg_rep1_cmd;
            my $grm_no_prm_fixed_effects_havg_rep1_cmd_status = system($grm_no_prm_fixed_effects_havg_rep1_cmd);
                ...
                while (my $row = <$fh>) {
                    ...
                    foreach my $encoded_trait (@header_cols_no_prm) {
                        if ($encoded_trait eq $trait_name_encoded_string) {
                            ...
                            my $value = $columns[$col_counter+1];
                            if (defined $value && $value ne '') {
                                $result_blup_data_s_havg_rep1->{$stock_name}->{$trait} = $value;
                                $seen_accession_names_havg_rep1{$stock_name}++;

                                if ($value < $genetic_effect_min_s_havg_rep1) {
                                    $genetic_effect_min_s_havg_rep1 = $value;
                                }
                                elsif ($value >= $genetic_effect_max_s_havg_rep1) {
                                    $genetic_effect_max_s_havg_rep1 = $value;
                                }

                                $genetic_effect_sum_s_havg_rep1 += abs($value);
                                $genetic_effect_sum_square_s_havg_rep1 = $genetic_effect_sum_square_s_havg_rep1 + $value*$value;
                            }
                        }
                        $col_counter++;
                    }
            ...
            open($fh_residual, '<', $stats_out_tempfile_residual) or die "Could not open file '$stats_out_tempfile_residual' $!";
                ...
                while (my $row = <$fh_residual>) {
                    ...
                    my $stock_name = $plot_id_map{$stock_id};
                    if (defined $residual && $residual ne '') {
                        $result_residual_data_s_havg_rep1->{$stock_name}->{$trait_name} = $residual;
                        $residual_sum_s_havg_rep1 += abs($residual);
                        $residual_sum_square_s_havg_rep1 = $residual_sum_square_s_havg_rep1 + $residual*$residual;
                    }
                    if (defined $fitted && $fitted ne '') {
                        $result_fitted_data_s_havg_rep1->{$stock_name}->{$trait_name} = $fitted;
                    }
                    $model_sum_square_residual_s_havg_rep1 = $model_sum_square_residual_s_havg_rep1 + $residual*$residual;
                }
            close($fh_residual);
            ...
            open($fh_varcomp, '<', $stats_out_tempfile_varcomp) or die "Could not open file '$stats_out_tempfile_varcomp' $!";
                ...
                while (my $row = <$fh_varcomp>) {
                    ...
                    push @varcomp_original_grm_fixed_effect_havg_rep1, \@columns;
                }
            close($fh_varcomp);
            ...
            open($fh_varcomp_h, '<', $stats_out_tempfile_vpredict) or die "Could not open file '$stats_out_tempfile_vpredict' $!";
                ...
                while (my $row = <$fh_varcomp_h>) {
                    ...
                    push @varcomp_h_grm_fixed_effect_havg_rep1, \@columns;
                }
            close($fh_varcomp_h);
            ...
            open($fh_fits, '<', $stats_out_tempfile_fits) or die "Could not open file '$stats_out_tempfile_fits' $!";
                ...
                while (my $row = <$fh_fits>) {
                    ...
                    push @fits_grm_fixed_effect_havg_rep1, \@columns;
                }
            close($fh_fits);

            my $result_blup_data_s_havg_rep2;
            my $genetic_effect_min_s_havg_rep2 = 10000000000;
            my $genetic_effect_max_s_havg_rep2 = -10000000000;
            my $genetic_effect_sum_s_havg_rep2 = 0;
            my $genetic_effect_sum_square_s_havg_rep2 = 0;
            my $result_residual_data_s_havg_rep2;
            my $residual_sum_s_havg_rep2 = 0;
            my $residual_sum_square_s_havg_rep2 = 0;
            my $result_fitted_data_s_havg_rep2;
            my $model_sum_square_residual_s_havg_rep2 = 0;
            my @varcomp_original_grm_fixed_effect_havg_rep2;
            my @varcomp_h_grm_fixed_effect_havg_rep2;
            my @fits_grm_fixed_effect_havg_rep2;
            my %seen_accession_names_havg_rep2;

            my $grm_no_prm_fixed_effects_havg_rep2_cmd = 'R -e "library(sommer); library(data.table); library(reshape2); library(ggplot2); library(GGally);
            mat <- data.frame(fread(\''.$stats_tempfile.'\', header=TRUE, sep=\',\'));
            mat_fixed <- data.frame(fread(\''.$analytics_protocol_data_tempfile29.'\', header=TRUE, sep=\',\'));
            geno_mat_3col <- data.frame(fread(\''.$grm_file.'\', header=FALSE, sep=\'\t\'));
            geno_mat <- acast(geno_mat_3col, V1~V2, value.var=\'V3\');
            ...
            "';
            print STDERR Dumper $grm_no_prm_fixed_effects_havg_rep2_cmd;
            my $grm_no_prm_fixed_effects_havg_rep2_cmd_status = system($grm_no_prm_fixed_effects_havg_rep2_cmd);
                ...
                while (my $row = <$fh>) {
                    ...
                    foreach my $encoded_trait (@header_cols_no_prm) {
                        if ($encoded_trait eq $trait_name_encoded_string) {
                            ...
                            my $value = $columns[$col_counter+1];
                            if (defined $value && $value ne '') {
                                $result_blup_data_s_havg_rep2->{$stock_name}->{$trait} = $value;
                                $seen_accession_names_havg_rep2{$stock_name}++;

                                if ($value < $genetic_effect_min_s_havg_rep2) {
                                    $genetic_effect_min_s_havg_rep2 = $value;
                                }
                                elsif ($value >= $genetic_effect_max_s_havg_rep2) {
                                    $genetic_effect_max_s_havg_rep2 = $value;
                                }

                                $genetic_effect_sum_s_havg_rep2 += abs($value);
                                $genetic_effect_sum_square_s_havg_rep2 = $genetic_effect_sum_square_s_havg_rep2 + $value*$value;
                            }
                        }
                        $col_counter++;
                    }
            ...
            open($fh_residual, '<', $stats_out_tempfile_residual) or die "Could not open file '$stats_out_tempfile_residual' $!";
                ...
                while (my $row = <$fh_residual>) {
                    ...
                    my $stock_name = $plot_id_map{$stock_id};
                    if (defined $residual && $residual ne '') {
                        $result_residual_data_s_havg_rep2->{$stock_name}->{$trait_name} = $residual;
                        $residual_sum_s_havg_rep2 += abs($residual);
                        $residual_sum_square_s_havg_rep2 = $residual_sum_square_s_havg_rep2 + $residual*$residual;
                    }
                    if (defined $fitted && $fitted ne '') {
                        $result_fitted_data_s_havg_rep2->{$stock_name}->{$trait_name} = $fitted;
                    }
                    $model_sum_square_residual_s_havg_rep2 = $model_sum_square_residual_s_havg_rep2 + $residual*$residual;
                }
            close($fh_residual);
            ...
            open($fh_varcomp, '<', $stats_out_tempfile_varcomp) or die "Could not open file '$stats_out_tempfile_varcomp' $!";
                ...
                while (my $row = <$fh_varcomp>) {
                    ...
                    push @varcomp_original_grm_fixed_effect_havg_rep2, \@columns;
                }
            close($fh_varcomp);
            ...
            open($fh_varcomp_h, '<', $stats_out_tempfile_vpredict) or die "Could not open file '$stats_out_tempfile_vpredict' $!";
                ...
                while (my $row = <$fh_varcomp_h>) {
                    ...
                    push @varcomp_h_grm_fixed_effect_havg_rep2, \@columns;
                }
            close($fh_varcomp_h);
            ...
            open($fh_fits, '<', $stats_out_tempfile_fits) or die "Could not open file '$stats_out_tempfile_fits' $!";
                ...
                while (my $row = <$fh_fits>) {
                    ...
                    push @fits_grm_fixed_effect_havg_rep2, \@columns;
                }
            close($fh_fits);

            my @seen_accession_names_havg_rep1_rep2;
            foreach my $a (sort keys %seen_accession_names_havg_rep1) {
                if (exists($seen_accession_names_havg_rep2{$a})) {
                    push @seen_accession_names_havg_rep1_rep2, $a;
                }
            }

            open(my $F_havg_gcorr, ">", $analytics_protocol_data_tempfile_havg_gcorr) || die "Can't open file ".$analytics_protocol_data_tempfile_havg_gcorr;
                print $F_havg_gcorr "accession_name,g_rep1,g_rep2\n";

                foreach my $stock_name (@seen_accession_names_havg_rep1_rep2) {
                    foreach my $trait_name (@sorted_trait_names) {
                        my $val1 = $result_blup_data_s_havg_rep1->{$stock_name}->{$trait_name} || 'NA';
                        my $val2 = $result_blup_data_s_havg_rep2->{$stock_name}->{$trait_name} || 'NA';
                        print $F_havg_gcorr "$stock_name,$val1,$val2\n";
                    }
                }
            close($F_havg_gcorr);
            ...
            my $result_blup_data_s_fmax_rep1;
            my $genetic_effect_min_s_fmax_rep1 = 10000000000;
            my $genetic_effect_max_s_fmax_rep1 = -10000000000;
            my $genetic_effect_sum_s_fmax_rep1 = 0;
            my $genetic_effect_sum_square_s_fmax_rep1 = 0;
            my $result_residual_data_s_fmax_rep1;
            my $residual_sum_s_fmax_rep1 = 0;
            my $residual_sum_square_s_fmax_rep1 = 0;
            my $result_fitted_data_s_fmax_rep1;
            my $model_sum_square_residual_s_fmax_rep1 = 0;
            my @varcomp_original_grm_fixed_effect_fmax_rep1;
            my @varcomp_h_grm_fixed_effect_fmax_rep1;
            my @fits_grm_fixed_effect_fmax_rep1;
            my %seen_accession_names_fmax_rep1;

            my $grm_no_prm_fixed_effects_fmax_rep1_cmd = 'R -e "library(sommer); library(data.table); library(reshape2); library(ggplot2); library(GGally);
            mat <- data.frame(fread(\''.$stats_tempfile.'\', header=TRUE, sep=\',\'));
            mat_fixed <- data.frame(fread(\''.$analytics_protocol_data_tempfile29.'\', header=TRUE, sep=\',\'));
            geno_mat_3col <- data.frame(fread(\''.$grm_file.'\', header=FALSE, sep=\'\t\'));
            geno_mat <- acast(geno_mat_3col, V1~V2, value.var=\'V3\');
            ...
            "';
            print STDERR Dumper $grm_no_prm_fixed_effects_fmax_rep1_cmd;
            my $grm_no_prm_fixed_effects_fmax_rep1_cmd_status = system($grm_no_prm_fixed_effects_fmax_rep1_cmd);
                ...
                while (my $row = <$fh>) {
                    ...
                    foreach my $encoded_trait (@header_cols_no_prm) {
                        if ($encoded_trait eq $trait_name_encoded_string) {
                            ...
                            my $value = $columns[$col_counter+1];
                            if (defined $value && $value ne '') {
                                $result_blup_data_s_fmax_rep1->{$stock_name}->{$trait} = $value;
                                $seen_accession_names_fmax_rep1{$stock_name}++;

                                if ($value < $genetic_effect_min_s_fmax_rep1) {
                                    $genetic_effect_min_s_fmax_rep1 = $value;
                                }
                                elsif ($value >= $genetic_effect_max_s_fmax_rep1) {
                                    $genetic_effect_max_s_fmax_rep1 = $value;
                                }

                                $genetic_effect_sum_s_fmax_rep1 += abs($value);
                                $genetic_effect_sum_square_s_fmax_rep1 = $genetic_effect_sum_square_s_fmax_rep1 + $value*$value;
                            }
                        }
                        $col_counter++;
                    }
            ...
            open($fh_residual, '<', $stats_out_tempfile_residual) or die "Could not open file '$stats_out_tempfile_residual' $!";
                ...
                while (my $row = <$fh_residual>) {
                    ...
                    my $stock_name = $plot_id_map{$stock_id};
                    if (defined $residual && $residual ne '') {
                        $result_residual_data_s_fmax_rep1->{$stock_name}->{$trait_name} = $residual;
                        $residual_sum_s_fmax_rep1 += abs($residual);
                        $residual_sum_square_s_fmax_rep1 = $residual_sum_square_s_fmax_rep1 + $residual*$residual;
                    }
                    if (defined $fitted && $fitted ne '') {
                        $result_fitted_data_s_fmax_rep1->{$stock_name}->{$trait_name} = $fitted;
                    }
                    $model_sum_square_residual_s_fmax_rep1 = $model_sum_square_residual_s_fmax_rep1 + $residual*$residual;
                }
            close($fh_residual);
            ...
            open($fh_varcomp, '<', $stats_out_tempfile_varcomp) or die "Could not open file '$stats_out_tempfile_varcomp' $!";
                ...
                while (my $row = <$fh_varcomp>) {
                    ...
                    push @varcomp_original_grm_fixed_effect_fmax_rep1, \@columns;
                }
            close($fh_varcomp);
            ...
            open($fh_varcomp_h, '<', $stats_out_tempfile_vpredict) or die "Could not open file '$stats_out_tempfile_vpredict' $!";
                ...
                while (my $row = <$fh_varcomp_h>) {
                    ...
                    push @varcomp_h_grm_fixed_effect_fmax_rep1, \@columns;
                }
            close($fh_varcomp_h);
            ...
            open($fh_fits, '<', $stats_out_tempfile_fits) or die "Could not open file '$stats_out_tempfile_fits' $!";
                ...
                while (my $row = <$fh_fits>) {
                    ...
                    push @fits_grm_fixed_effect_fmax_rep1, \@columns;
                }
            close($fh_fits);

            my $result_blup_data_s_fmax_rep2;
            my $genetic_effect_min_s_fmax_rep2 = 10000000000;
            my $genetic_effect_max_s_fmax_rep2 = -10000000000;
            my $genetic_effect_sum_s_fmax_rep2 = 0;
            my $genetic_effect_sum_square_s_fmax_rep2 = 0;
            my $result_residual_data_s_fmax_rep2;
            my $residual_sum_s_fmax_rep2 = 0;
            my $residual_sum_square_s_fmax_rep2 = 0;
            my $result_fitted_data_s_fmax_rep2;
            my $model_sum_square_residual_s_fmax_rep2 = 0;
            my @varcomp_original_grm_fixed_effect_fmax_rep2;
            my @varcomp_h_grm_fixed_effect_fmax_rep2;
            my @fits_grm_fixed_effect_fmax_rep2;
            my %seen_accession_names_fmax_rep2;

            my $grm_no_prm_fixed_effects_fmax_rep2_cmd = 'R -e "library(sommer); library(data.table); library(reshape2); library(ggplot2); library(GGally);
            mat <- data.frame(fread(\''.$stats_tempfile.'\', header=TRUE, sep=\',\'));
            mat_fixed <- data.frame(fread(\''.$analytics_protocol_data_tempfile29.'\', header=TRUE, sep=\',\'));
            geno_mat_3col <- data.frame(fread(\''.$grm_file.'\', header=FALSE, sep=\'\t\'));
            geno_mat <- acast(geno_mat_3col, V1~V2, value.var=\'V3\');
            ...
            "';
            print STDERR Dumper $grm_no_prm_fixed_effects_fmax_rep2_cmd;
            my $grm_no_prm_fixed_effects_fmax_rep2_cmd_status = system($grm_no_prm_fixed_effects_fmax_rep2_cmd);
                ...
                while (my $row = <$fh>) {
                    ...
                    foreach my $encoded_trait (@header_cols_no_prm) {
                        if ($encoded_trait eq $trait_name_encoded_string) {
                            ...
                            my $value = $columns[$col_counter+1];
                            if (defined $value && $value ne '') {
                                $result_blup_data_s_fmax_rep2->{$stock_name}->{$trait} = $value;
                                $seen_accession_names_fmax_rep2{$stock_name}++;

                                if ($value < $genetic_effect_min_s_fmax_rep2) {
                                    $genetic_effect_min_s_fmax_rep2 = $value;
                                }
                                elsif ($value >= $genetic_effect_max_s_fmax_rep2) {
                                    $genetic_effect_max_s_fmax_rep2 = $value;
                                }

                                $genetic_effect_sum_s_fmax_rep2 += abs($value);
                                $genetic_effect_sum_square_s_fmax_rep2 = $genetic_effect_sum_square_s_fmax_rep2 + $value*$value;
                            }
                        }
                        $col_counter++;
                    }
            ...
            open($fh_residual, '<', $stats_out_tempfile_residual) or die "Could not open file '$stats_out_tempfile_residual' $!";
                ...
                while (my $row = <$fh_residual>) {
                    ...
                    my $stock_name = $plot_id_map{$stock_id};
                    if (defined $residual && $residual ne '') {
                        $result_residual_data_s_fmax_rep2->{$stock_name}->{$trait_name} = $residual;
                        $residual_sum_s_fmax_rep2 += abs($residual);
                        $residual_sum_square_s_fmax_rep2 = $residual_sum_square_s_fmax_rep2 + $residual*$residual;
                    }
                    if (defined $fitted && $fitted ne '') {
                        $result_fitted_data_s_fmax_rep2->{$stock_name}->{$trait_name} = $fitted;
                    }
                    $model_sum_square_residual_s_fmax_rep2 = $model_sum_square_residual_s_fmax_rep2 + $residual*$residual;
                }
            close($fh_residual);
            ...
            open($fh_varcomp, '<', $stats_out_tempfile_varcomp) or die "Could not open file '$stats_out_tempfile_varcomp' $!";
                ...
                while (my $row = <$fh_varcomp>) {
                    ...
                    push @varcomp_original_grm_fixed_effect_fmax_rep2, \@columns;
                }
            close($fh_varcomp);
            ...
            open($fh_varcomp_h, '<', $stats_out_tempfile_vpredict) or die "Could not open file '$stats_out_tempfile_vpredict' $!";
                ...
                while (my $row = <$fh_varcomp_h>) {
                    ...
                    push @varcomp_h_grm_fixed_effect_fmax_rep2, \@columns;
                }
            close($fh_varcomp_h);
            ...
            open($fh_fits, '<', $stats_out_tempfile_fits) or die "Could not open file '$stats_out_tempfile_fits' $!";
                ...
                while (my $row = <$fh_fits>) {
                    ...
                    push @fits_grm_fixed_effect_fmax_rep2, \@columns;
                }
            close($fh_fits);

            my @seen_accession_names_fmax_rep1_rep2;
            foreach my $a (sort keys %seen_accession_names_fmax_rep1) {
                if (exists($seen_accession_names_fmax_rep2{$a})) {
                    push @seen_accession_names_fmax_rep1_rep2, $a;
                }
            }

            open(my $F_fmax_gcorr, ">", $analytics_protocol_data_tempfile_fmax_gcorr) || die "Can't open file ".$analytics_protocol_data_tempfile_fmax_gcorr;
                print $F_fmax_gcorr "accession_name,g_rep1,g_rep2\n";

                foreach my $stock_name (@seen_accession_names_fmax_rep1_rep2) {
                    foreach my $trait_name (@sorted_trait_names) {
                        my $val1 = $result_blup_data_s_fmax_rep1->{$stock_name}->{$trait_name} || 'NA';
                        my $val2 = $result_blup_data_s_fmax_rep2->{$stock_name}->{$trait_name} || 'NA';
                        print $F_fmax_gcorr "$stock_name,$val1,$val2\n";
                    }
                }
            close($F_fmax_gcorr);
            ...
            my $result_blup_data_s_fmin_rep1;
            my $genetic_effect_min_s_fmin_rep1 = 10000000000;
            my $genetic_effect_max_s_fmin_rep1 = -10000000000;
            my $genetic_effect_sum_s_fmin_rep1 = 0;
            my $genetic_effect_sum_square_s_fmin_rep1 = 0;
            my $result_residual_data_s_fmin_rep1;
            my $residual_sum_s_fmin_rep1 = 0;
            my $residual_sum_square_s_fmin_rep1 = 0;
            my $result_fitted_data_s_fmin_rep1;
            my $model_sum_square_residual_s_fmin_rep1 = 0;
            my @varcomp_original_grm_fixed_effect_fmin_rep1;
            my @varcomp_h_grm_fixed_effect_fmin_rep1;
            my @fits_grm_fixed_effect_fmin_rep1;
            my %seen_accession_names_fmin_rep1;

            my $grm_no_prm_fixed_effects_fmin_rep1_cmd = 'R -e "library(sommer); library(data.table); library(reshape2); library(ggplot2); library(GGally);
            mat <- data.frame(fread(\''.$stats_tempfile.'\', header=TRUE, sep=\',\'));
            mat_fixed <- data.frame(fread(\''.$analytics_protocol_data_tempfile29.'\', header=TRUE, sep=\',\'));
            geno_mat_3col <- data.frame(fread(\''.$grm_file.'\', header=FALSE, sep=\'\t\'));
            geno_mat <- acast(geno_mat_3col, V1~V2, value.var=\'V3\');
            ...
            "';
            print STDERR Dumper $grm_no_prm_fixed_effects_fmin_rep1_cmd;
            my $grm_no_prm_fixed_effects_fmin_rep1_cmd_status = system($grm_no_prm_fixed_effects_fmin_rep1_cmd);
                ...
                while (my $row = <$fh>) {
                    ...
                    foreach my $encoded_trait (@header_cols_no_prm) {
                        if ($encoded_trait eq $trait_name_encoded_string) {
                            ...
                            my $value = $columns[$col_counter+1];
                            if (defined $value && $value ne '') {
                                $result_blup_data_s_fmin_rep1->{$stock_name}->{$trait} = $value;
                                $seen_accession_names_fmin_rep1{$stock_name}++;

                                if ($value < $genetic_effect_min_s_fmin_rep1) {
                                    $genetic_effect_min_s_fmin_rep1 = $value;
                                }
                                elsif ($value >= $genetic_effect_max_s_fmin_rep1) {
                                    $genetic_effect_max_s_fmin_rep1 = $value;
                                }

                                $genetic_effect_sum_s_fmin_rep1 += abs($value);
                                $genetic_effect_sum_square_s_fmin_rep1 = $genetic_effect_sum_square_s_fmin_rep1 + $value*$value;
                            }
                        }
                        $col_counter++;
                    }
            ...
            open($fh_residual, '<', $stats_out_tempfile_residual) or die "Could not open file '$stats_out_tempfile_residual' $!";
                ...
                while (my $row = <$fh_residual>) {
                    ...
                    my $stock_name = $plot_id_map{$stock_id};
                    if (defined $residual && $residual ne '') {
                        $result_residual_data_s_fmin_rep1->{$stock_name}->{$trait_name} = $residual;
                        $residual_sum_s_fmin_rep1 += abs($residual);
                        $residual_sum_square_s_fmin_rep1 = $residual_sum_square_s_fmin_rep1 + $residual*$residual;
                    }
                    if (defined $fitted && $fitted ne '') {
                        $result_fitted_data_s_fmin_rep1->{$stock_name}->{$trait_name} = $fitted;
                    }
                    $model_sum_square_residual_s_fmin_rep1 = $model_sum_square_residual_s_fmin_rep1 + $residual*$residual;
                }
            close($fh_residual);
            ...
            open($fh_varcomp, '<', $stats_out_tempfile_varcomp) or die "Could not open file '$stats_out_tempfile_varcomp' $!";
                ...
                while (my $row = <$fh_varcomp>) {
                    ...
                    push @varcomp_original_grm_fixed_effect_fmin_rep1, \@columns;
                }
            close($fh_varcomp);
            ...
            open($fh_varcomp_h, '<', $stats_out_tempfile_vpredict) or die "Could not open file '$stats_out_tempfile_vpredict' $!";
                ...
                while (my $row = <$fh_varcomp_h>) {
                    ...
                    push @varcomp_h_grm_fixed_effect_fmin_rep1, \@columns;
                }
            close($fh_varcomp_h);
            ...
            open($fh_fits, '<', $stats_out_tempfile_fits) or die "Could not open file '$stats_out_tempfile_fits' $!";
                ...
                while (my $row = <$fh_fits>) {
                    ...
                    push @fits_grm_fixed_effect_fmin_rep1, \@columns;
                }
            close($fh_fits);

            my $result_blup_data_s_fmin_rep2;
            my $genetic_effect_min_s_fmin_rep2 = 10000000000;
            my $genetic_effect_max_s_fmin_rep2 = -10000000000;
            my $genetic_effect_sum_s_fmin_rep2 = 0;
            my $genetic_effect_sum_square_s_fmin_rep2 = 0;
            my $result_residual_data_s_fmin_rep2;
            my $residual_sum_s_fmin_rep2 = 0;
            my $residual_sum_square_s_fmin_rep2 = 0;
            my $result_fitted_data_s_fmin_rep2;
            my $model_sum_square_residual_s_fmin_rep2 = 0;
            my @varcomp_original_grm_fixed_effect_fmin_rep2;
            my @varcomp_h_grm_fixed_effect_fmin_rep2;
            my @fits_grm_fixed_effect_fmin_rep2;
            my %seen_accession_names_fmin_rep2;

            my $grm_no_prm_fixed_effects_fmin_rep2_cmd = 'R -e "library(sommer); library(data.table); library(reshape2); library(ggplot2); library(GGally);
            mat <- data.frame(fread(\''.$stats_tempfile.'\', header=TRUE, sep=\',\'));
            mat_fixed <- data.frame(fread(\''.$analytics_protocol_data_tempfile29.'\', header=TRUE, sep=\',\'));
            geno_mat_3col <- data.frame(fread(\''.$grm_file.'\', header=FALSE, sep=\'\t\'));
            geno_mat <- acast(geno_mat_3col, V1~V2, value.var=\'V3\');
            ...
            "';
            print STDERR Dumper $grm_no_prm_fixed_effects_fmin_rep2_cmd;
            my $grm_no_prm_fixed_effects_fmin_rep2_cmd_status = system($grm_no_prm_fixed_effects_fmin_rep2_cmd);
                ...
                while (my $row = <$fh>) {
                    ...
                    foreach my $encoded_trait (@header_cols_no_prm) {
                        if ($encoded_trait eq $trait_name_encoded_string) {
                            ...
                            my $value = $columns[$col_counter+1];
                            if (defined $value && $value ne '') {
                                $result_blup_data_s_fmin_rep2->{$stock_name}->{$trait} = $value;
                                $seen_accession_names_fmin_rep2{$stock_name}++;

                                if ($value < $genetic_effect_min_s_fmin_rep2) {
                                    $genetic_effect_min_s_fmin_rep2 = $value;
                                }
                                elsif ($value >= $genetic_effect_max_s_fmin_rep2) {
                                    $genetic_effect_max_s_fmin_rep2 = $value;
                                }

                                $genetic_effect_sum_s_fmin_rep2 += abs($value);
                                $genetic_effect_sum_square_s_fmin_rep2 = $genetic_effect_sum_square_s_fmin_rep2 + $value*$value;
                            }
                        }
                        $col_counter++;
                    }
            ...
            open($fh_residual, '<', $stats_out_tempfile_residual) or die "Could not open file '$stats_out_tempfile_residual' $!";
                ...
                while (my $row = <$fh_residual>) {
                    ...
                    my $stock_name = $plot_id_map{$stock_id};
                    if (defined $residual && $residual ne '') {
                        $result_residual_data_s_fmin_rep2->{$stock_name}->{$trait_name} = $residual;
                        $residual_sum_s_fmin_rep2 += abs($residual);
                        $residual_sum_square_s_fmin_rep2 = $residual_sum_square_s_fmin_rep2 + $residual*$residual;
                    }
                    if (defined $fitted && $fitted ne '') {
                        $result_fitted_data_s_fmin_rep2->{$stock_name}->{$trait_name} = $fitted;
                    }
                    $model_sum_square_residual_s_fmin_rep2 = $model_sum_square_residual_s_fmin_rep2 + $residual*$residual;
                }
            close($fh_residual);
            ...
            open($fh_varcomp, '<', $stats_out_tempfile_varcomp) or die "Could not open file '$stats_out_tempfile_varcomp' $!";
                ...
                while (my $row = <$fh_varcomp>) {
                    ...
                    push @varcomp_original_grm_fixed_effect_fmin_rep2, \@columns;
                }
            close($fh_varcomp);
            ...
            open($fh_varcomp_h, '<', $stats_out_tempfile_vpredict) or die "Could not open file '$stats_out_tempfile_vpredict' $!";
                ...
                while (my $row = <$fh_varcomp_h>) {
                    ...
                    push @varcomp_h_grm_fixed_effect_fmin_rep2, \@columns;
                }
            close($fh_varcomp_h);
            ...
            open($fh_fits, '<', $stats_out_tempfile_fits) or die "Could not open file '$stats_out_tempfile_fits' $!";
                ...
                while (my $row = <$fh_fits>) {
                    ...
                    push @fits_grm_fixed_effect_fmin_rep2, \@columns;
                }
            close($fh_fits);

            my @seen_accession_names_fmin_rep1_rep2;
            foreach my $a (sort keys %seen_accession_names_fmin_rep1) {
                if (exists($seen_accession_names_fmin_rep2{$a})) {
                    push @seen_accession_names_fmin_rep1_rep2, $a;
                }
            }

            open(my $F_fmin_gcorr, ">", $analytics_protocol_data_tempfile_fmin_gcorr) || die "Can't open file ".$analytics_protocol_data_tempfile_fmin_gcorr;
                print $F_fmin_gcorr "accession_name,g_rep1,g_rep2\n";

                foreach my $stock_name (@seen_accession_names_fmin_rep1_rep2) {
                    foreach my $trait_name (@sorted_trait_names) {
                        my $val1 = $result_blup_data_s_fmin_rep1->{$stock_name}->{$trait_name} || 'NA';
                        my $val2 = $result_blup_data_s_fmin_rep2->{$stock_name}->{$trait_name} || 'NA';
                        print $F_fmin_gcorr "$stock_name,$val1,$val2\n";
                    }
                }
            close($F_fmin_gcorr);
            ...
            my $result_blup_data_s_grm_rep1;
            my $genetic_effect_min_s_grm_rep1 = 10000000000;
            my $genetic_effect_max_s_grm_rep1 = -10000000000;
            my $genetic_effect_sum_s_grm_rep1 = 0;
            my $genetic_effect_sum_square_s_grm_rep1 = 0;
            my $result_residual_data_s_grm_rep1;
            my $residual_sum_s_grm_rep1 = 0;
            my $residual_sum_square_s_grm_rep1 = 0;
            my $result_fitted_data_s_grm_rep1;
            my $model_sum_square_residual_s_grm_rep1 = 0;
            my @varcomp_original_grm_fixed_effect_grm_rep1;
            my @varcomp_h_grm_fixed_effect_grm_rep1;
            my @fits_grm_fixed_effect_grm_rep1;
            my %seen_accession_names_grm_rep1;

            my $grm_no_prm_fixed_effects_grm_rep1_cmd = 'R -e "library(sommer); library(data.table); library(reshape2); library(ggplot2); library(GGally);
            mat <- data.frame(fread(\''.$stats_tempfile.'\', header=TRUE, sep=\',\'));
            geno_mat_3col <- data.frame(fread(\''.$grm_file.'\', header=FALSE, sep=\'\t\'));
            geno_mat <- acast(geno_mat_3col, V1~V2, value.var=\'V3\');
            geno_mat[is.na(geno_mat)] <- 0;
            ...
            "';
            print STDERR Dumper $grm_no_prm_fixed_effects_grm_rep1_cmd;
            my $grm_no_prm_fixed_effects_grm_rep1_cmd_status = system($grm_no_prm_fixed_effects_grm_rep1_cmd);
                ...
                while (my $row = <$fh>) {
                    ...
                    foreach my $encoded_trait (@header_cols_no_prm) {
                        if ($encoded_trait eq $trait_name_encoded_string) {
                            ...
                            my $value = $columns[$col_counter+1];
                            if (defined $value && $value ne '') {
                                $result_blup_data_s_grm_rep1->{$stock_name}->{$trait} = $value;
                                $seen_accession_names_grm_rep1{$stock_name}++;

                                if ($value < $genetic_effect_min_s_grm_rep1) {
                                    $genetic_effect_min_s_grm_rep1 = $value;
                                }
                                elsif ($value >= $genetic_effect_max_s_grm_rep1) {
                                    $genetic_effect_max_s_grm_rep1 = $value;
                                }

                                $genetic_effect_sum_s_grm_rep1 += abs($value);
                                $genetic_effect_sum_square_s_grm_rep1 = $genetic_effect_sum_square_s_grm_rep1 + $value*$value;
                            }
                        }
                        $col_counter++;
                    }
            ...
            open($fh_residual, '<', $stats_out_tempfile_residual) or die "Could not open file '$stats_out_tempfile_residual' $!";
                ...
                while (my $row = <$fh_residual>) {
                    ...
                    my $stock_name = $plot_id_map{$stock_id};
                    if (defined $residual && $residual ne '') {
                        $result_residual_data_s_grm_rep1->{$stock_name}->{$trait_name} = $residual;
                        $residual_sum_s_grm_rep1 += abs($residual);
                        $residual_sum_square_s_grm_rep1 = $residual_sum_square_s_grm_rep1 + $residual*$residual;
                    }
                    if (defined $fitted && $fitted ne '') {
                        $result_fitted_data_s_grm_rep1->{$stock_name}->{$trait_name} = $fitted;
                    }
                    $model_sum_square_residual_s_grm_rep1 = $model_sum_square_residual_s_grm_rep1 + $residual*$residual;
                }
            close($fh_residual);
            ...
            open($fh_varcomp, '<', $stats_out_tempfile_varcomp) or die "Could not open file '$stats_out_tempfile_varcomp' $!";
                ...
                while (my $row = <$fh_varcomp>) {
                    ...
                    push @varcomp_original_grm_fixed_effect_grm_rep1, \@columns;
                }
            close($fh_varcomp);
            ...
            open($fh_varcomp_h, '<', $stats_out_tempfile_vpredict) or die "Could not open file '$stats_out_tempfile_vpredict' $!";
                ...
                while (my $row = <$fh_varcomp_h>) {
                    ...
                    push @varcomp_h_grm_fixed_effect_grm_rep1, \@columns;
                }
            close($fh_varcomp_h);
            ...
            open($fh_fits, '<', $stats_out_tempfile_fits) or die "Could not open file '$stats_out_tempfile_fits' $!";
                ...
                while (my $row = <$fh_fits>) {
                    ...
                    push @fits_grm_fixed_effect_grm_rep1, \@columns;
                }
            close($fh_fits);

            my $result_blup_data_s_grm_rep2;
            my $genetic_effect_min_s_grm_rep2 = 10000000000;
            my $genetic_effect_max_s_grm_rep2 = -10000000000;
            my $genetic_effect_sum_s_grm_rep2 = 0;
            my $genetic_effect_sum_square_s_grm_rep2 = 0;
            my $result_residual_data_s_grm_rep2;
            my $residual_sum_s_grm_rep2 = 0;
            my $residual_sum_square_s_grm_rep2 = 0;
            my $result_fitted_data_s_grm_rep2;
            my $model_sum_square_residual_s_grm_rep2 = 0;
            my @varcomp_original_grm_fixed_effect_grm_rep2;
            my @varcomp_h_grm_fixed_effect_grm_rep2;
            my @fits_grm_fixed_effect_grm_rep2;
            my %seen_accession_names_grm_rep2;

            my $grm_no_prm_fixed_effects_grm_rep2_cmd = 'R -e "library(sommer); library(data.table); library(reshape2); library(ggplot2); library(GGally);
            mat <- data.frame(fread(\''.$stats_tempfile.'\', header=TRUE, sep=\',\'));
            geno_mat_3col <- data.frame(fread(\''.$grm_file.'\', header=FALSE, sep=\'\t\'));
            geno_mat <- acast(geno_mat_3col, V1~V2, value.var=\'V3\');
            geno_mat[is.na(geno_mat)] <- 0;
            ...
            "';
            print STDERR Dumper $grm_no_prm_fixed_effects_grm_rep2_cmd;
            my $grm_no_prm_fixed_effects_grm_rep2_cmd_status = system($grm_no_prm_fixed_effects_grm_rep2_cmd);
                ...
                while (my $row = <$fh>) {
                    ...
                    foreach my $encoded_trait (@header_cols_no_prm) {
                        if ($encoded_trait eq $trait_name_encoded_string) {
                            ...
                            my $value = $columns[$col_counter+1];
                            if (defined $value && $value ne '') {
                                $result_blup_data_s_grm_rep2->{$stock_name}->{$trait} = $value;
                                $seen_accession_names_grm_rep2{$stock_name}++;

                                if ($value < $genetic_effect_min_s_grm_rep2) {
                                    $genetic_effect_min_s_grm_rep2 = $value;
                                }
                                elsif ($value >= $genetic_effect_max_s_grm_rep2) {
                                    $genetic_effect_max_s_grm_rep2 = $value;
                                }

                                $genetic_effect_sum_s_grm_rep2 += abs($value);
                                $genetic_effect_sum_square_s_grm_rep2 = $genetic_effect_sum_square_s_grm_rep2 + $value*$value;
                            }
                        }
                        $col_counter++;
                    }
            ...
            open($fh_residual, '<', $stats_out_tempfile_residual) or die "Could not open file '$stats_out_tempfile_residual' $!";
                ...
                while (my $row = <$fh_residual>) {
                    ...
                    my $stock_name = $plot_id_map{$stock_id};
                    if (defined $residual && $residual ne '') {
                        $result_residual_data_s_grm_rep2->{$stock_name}->{$trait_name} = $residual;
                        $residual_sum_s_grm_rep2 += abs($residual);
                        $residual_sum_square_s_grm_rep2 = $residual_sum_square_s_grm_rep2 + $residual*$residual;
                    }
                    if (defined $fitted && $fitted ne '') {
                        $result_fitted_data_s_grm_rep2->{$stock_name}->{$trait_name} = $fitted;
                    }
                    $model_sum_square_residual_s_grm_rep2 = $model_sum_square_residual_s_grm_rep2 + $residual*$residual;
                }
            close($fh_residual);
            ...
            open($fh_varcomp, '<', $stats_out_tempfile_varcomp) or die "Could not open file '$stats_out_tempfile_varcomp' $!";
                ...
                while (my $row = <$fh_varcomp>) {
                    ...
                    push @varcomp_original_grm_fixed_effect_grm_rep2, \@columns;
                }
            close($fh_varcomp);
            ...
            open($fh_varcomp_h, '<', $stats_out_tempfile_vpredict) or die "Could not open file '$stats_out_tempfile_vpredict' $!";
                ...
                while (my $row = <$fh_varcomp_h>) {
                    ...
                    push @varcomp_h_grm_fixed_effect_grm_rep2, \@columns;
                }
            close($fh_varcomp_h);
            ...
            open($fh_fits, '<', $stats_out_tempfile_fits) or die "Could not open file '$stats_out_tempfile_fits' $!";
                ...
                while (my $row = <$fh_fits>) {
                    ...
                    push @fits_grm_fixed_effect_grm_rep2, \@columns;
                }
            close($fh_fits);

            my @seen_accession_names_grm_rep1_rep2;
            foreach my $a (sort keys %seen_accession_names_grm_rep1) {
                if (exists($seen_accession_names_grm_rep2{$a})) {
                    push @seen_accession_names_grm_rep1_rep2, $a;
                }
            }

            open(my $F_grm_gcorr, ">", $analytics_protocol_data_tempfile_grm_gcorr) || die "Can't open file ".$analytics_protocol_data_tempfile_grm_gcorr;
                print $F_grm_gcorr "accession_name,g_rep1,g_rep2\n";

                foreach my $stock_name (@seen_accession_names_grm_rep1_rep2) {
                    foreach my $trait_name (@sorted_trait_names) {
                        my $val1 = $result_blup_data_s_grm_rep1->{$stock_name}->{$trait_name} || 'NA';
                        my $val2 = $result_blup_data_s_grm_rep2->{$stock_name}->{$trait_name} || 'NA';
                        print $F_grm_gcorr "$stock_name,$val1,$val2\n";
                    }
                }
            close($F_grm_gcorr);
            ...
            my $result_blup_data_s_prm_rep1;
            my $genetic_effect_min_s_prm_rep1 = 10000000000;
            my $genetic_effect_max_s_prm_rep1 = -10000000000;
            my $genetic_effect_sum_s_prm_rep1 = 0;
            my $genetic_effect_sum_square_s_prm_rep1 = 0;
            my $result_residual_data_s_prm_rep1;
            my $residual_sum_s_prm_rep1 = 0;
            my $residual_sum_square_s_prm_rep1 = 0;
            my $result_fitted_data_s_prm_rep1;
            my $model_sum_square_residual_s_prm_rep1 = 0;
            my @varcomp_original_grm_fixed_effect_prm_rep1;
            my @varcomp_h_grm_fixed_effect_prm_rep1;
            my @fits_grm_fixed_effect_prm_rep1;
            my %seen_accession_names_prm_rep1;

            my $prm_fixed_effects_prm_rep1_cmd = 'R -e "library(sommer); library(data.table); library(reshape2); library(ggplot2); library(GGally);
            mat <- data.frame(fread(\''.$stats_tempfile.'\', header=TRUE, sep=\',\'));
            geno_mat_3col <- data.frame(fread(\''.$grm_file.'\', header=FALSE, sep=\'\t\'));
            prm_mat_cols <- data.frame(fread(\''.$analytics_protocol_data_tempfile27.'\', header=FALSE, sep=\',\'));
            geno_mat <- acast(geno_mat_3col, V1~V2, value.var=\'V3\');
            ...
            "';
            print STDERR Dumper $prm_fixed_effects_prm_rep1_cmd;
            my $prm_fixed_effects_prm_rep1_cmd_status = system($prm_fixed_effects_prm_rep1_cmd);
                ...
                while (my $row = <$fh>) {
                    ...
                    foreach my $encoded_trait (@header_cols_no_prm) {
                        if ($encoded_trait eq $trait_name_encoded_string) {
                            ...
                            my $value = $columns[$col_counter+1];
                            if (defined $value && $value ne '') {
                                $result_blup_data_s_prm_rep1->{$stock_name}->{$trait} = $value;
                                $seen_accession_names_prm_rep1{$stock_name}++;

                                if ($value < $genetic_effect_min_s_prm_rep1) {
                                    $genetic_effect_min_s_prm_rep1 = $value;
                                }
                                elsif ($value >= $genetic_effect_max_s_prm_rep1) {
                                    $genetic_effect_max_s_prm_rep1 = $value;
                                }

                                $genetic_effect_sum_s_prm_rep1 += abs($value);
                                $genetic_effect_sum_square_s_prm_rep1 = $genetic_effect_sum_square_s_prm_rep1 + $value*$value;
                            }
                        }
                        $col_counter++;
                    }
            ...
            open($fh_residual, '<', $stats_out_tempfile_residual) or die "Could not open file '$stats_out_tempfile_residual' $!";
                ...
                while (my $row = <$fh_residual>) {
                    ...
                    my $stock_name = $plot_id_map{$stock_id};
                    if (defined $residual && $residual ne '') {
                        $result_residual_data_s_prm_rep1->{$stock_name}->{$trait_name} = $residual;
                        $residual_sum_s_prm_rep1 += abs($residual);
                        $residual_sum_square_s_prm_rep1 = $residual_sum_square_s_prm_rep1 + $residual*$residual;
                    }
                    if (defined $fitted && $fitted ne '') {
                        $result_fitted_data_s_prm_rep1->{$stock_name}->{$trait_name} = $fitted;
                    }
                    $model_sum_square_residual_s_prm_rep1 = $model_sum_square_residual_s_prm_rep1 + $residual*$residual;
                }
            close($fh_residual);
            ...
            open($fh_varcomp, '<', $stats_out_tempfile_varcomp) or die "Could not open file '$stats_out_tempfile_varcomp' $!";
                ...
                while (my $row = <$fh_varcomp>) {
                    ...
                    push @varcomp_original_grm_fixed_effect_prm_rep1, \@columns;
                }
            close($fh_varcomp);
            ...
            open($fh_varcomp_h, '<', $stats_out_tempfile_vpredict) or die "Could not open file '$stats_out_tempfile_vpredict' $!";
                ...
                while (my $row = <$fh_varcomp_h>) {
                    ...
                    push @varcomp_h_grm_fixed_effect_prm_rep1, \@columns;
                }
            close($fh_varcomp_h);
            ...
            open($fh_fits, '<', $stats_out_tempfile_fits) or die "Could not open file '$stats_out_tempfile_fits' $!";
                ...
                while (my $row = <$fh_fits>) {
                    ...
                    push @fits_grm_fixed_effect_prm_rep1, \@columns;
                }
            close($fh_fits);

            my $result_blup_data_s_prm_rep2;
            my $genetic_effect_min_s_prm_rep2 = 10000000000;
            my $genetic_effect_max_s_prm_rep2 = -10000000000;
            my $genetic_effect_sum_s_prm_rep2 = 0;
            my $genetic_effect_sum_square_s_prm_rep2 = 0;
            my $result_residual_data_s_prm_rep2;
            my $residual_sum_s_prm_rep2 = 0;
            my $residual_sum_square_s_prm_rep2 = 0;
            my $result_fitted_data_s_prm_rep2;
            my $model_sum_square_residual_s_prm_rep2 = 0;
            my @varcomp_original_grm_fixed_effect_prm_rep2;
            my @varcomp_h_grm_fixed_effect_prm_rep2;
            my @fits_grm_fixed_effect_prm_rep2;
            my %seen_accession_names_prm_rep2;

            my $prm_fixed_effects_prm_rep2_cmd = 'R -e "library(sommer); library(data.table); library(reshape2); library(ggplot2); library(GGally);
            mat <- data.frame(fread(\''.$stats_tempfile.'\', header=TRUE, sep=\',\'));
            geno_mat_3col <- data.frame(fread(\''.$grm_file.'\', header=FALSE, sep=\'\t\'));
            prm_mat_cols <- data.frame(fread(\''.$analytics_protocol_data_tempfile27.'\', header=FALSE, sep=\',\'));
            geno_mat <- acast(geno_mat_3col, V1~V2, value.var=\'V3\');
            ...
            "';
            print STDERR Dumper $prm_fixed_effects_prm_rep2_cmd;
            my $prm_fixed_effects_prm_rep2_cmd_status = system($prm_fixed_effects_prm_rep2_cmd);
                ...
                while (my $row = <$fh>) {
                    ...
                    foreach my $encoded_trait (@header_cols_no_prm) {
                        if ($encoded_trait eq $trait_name_encoded_string) {
                            ...
                            my $value = $columns[$col_counter+1];
                            if (defined $value && $value ne '') {
                                $result_blup_data_s_prm_rep2->{$stock_name}->{$trait} = $value;
                                $seen_accession_names_prm_rep2{$stock_name}++;

                                if ($value < $genetic_effect_min_s_prm_rep2) {
                                    $genetic_effect_min_s_prm_rep2 = $value;
                                }
                                elsif ($value >= $genetic_effect_max_s_prm_rep2) {
                                    $genetic_effect_max_s_prm_rep2 = $value;
                                }

                                $genetic_effect_sum_s_prm_rep2 += abs($value);
                                $genetic_effect_sum_square_s_prm_rep2 = $genetic_effect_sum_square_s_prm_rep2 + $value*$value;
                            }
                        }
                        $col_counter++;
                    }
            ...
            open($fh_residual, '<', $stats_out_tempfile_residual) or die "Could not open file '$stats_out_tempfile_residual' $!";
                ...
                while (my $row = <$fh_residual>) {
                    ...
                    my $stock_name = $plot_id_map{$stock_id};
                    if (defined $residual && $residual ne '') {
                        $result_residual_data_s_prm_rep2->{$stock_name}->{$trait_name} = $residual;
                        $residual_sum_s_prm_rep2 += abs($residual);
                        $residual_sum_square_s_prm_rep2 = $residual_sum_square_s_grm_rep2 + $residual*$residual;
                    }
                    if (defined $fitted && $fitted ne '') {
                        $result_fitted_data_s_prm_rep2->{$stock_name}->{$trait_name} = $fitted;
                    }
                    $model_sum_square_residual_s_prm_rep2 = $model_sum_square_residual_s_prm_rep2 + $residual*$residual;
                }
            close($fh_residual);
            ...
            open($fh_varcomp, '<', $stats_out_tempfile_varcomp) or die "Could not open file '$stats_out_tempfile_varcomp' $!";
                ...
                while (my $row = <$fh_varcomp>) {
                    ...
                    push @varcomp_original_grm_fixed_effect_prm_rep2, \@columns;
                }
            close($fh_varcomp);
            ...
            open($fh_varcomp_h, '<', $stats_out_tempfile_vpredict) or die "Could not open file '$stats_out_tempfile_vpredict' $!";
                ...
                while (my $row = <$fh_varcomp_h>) {
                    ...
                    push @varcomp_h_grm_fixed_effect_prm_rep2, \@columns;
                }
            close($fh_varcomp_h);
            ...
            open($fh_fits, '<', $stats_out_tempfile_fits) or die "Could not open file '$stats_out_tempfile_fits' $!";
                ...
                while (my $row = <$fh_fits>) {
                    ...
                    push @fits_grm_fixed_effect_prm_rep2, \@columns;
                }
            close($fh_fits);

            my @seen_accession_names_prm_rep1_rep2;
            foreach my $a (sort keys %seen_accession_names_prm_rep1) {
                if (exists($seen_accession_names_prm_rep2{$a})) {
                    push @seen_accession_names_prm_rep1_rep2, $a;
                }
            }

            open(my $F_prm_gcorr, ">", $analytics_protocol_data_tempfile_prm_gcorr) || die "Can't open file ".$analytics_protocol_data_tempfile_prm_gcorr;
                print $F_prm_gcorr "accession_name,g_rep1,g_rep2\n";

                foreach my $stock_name (@seen_accession_names_prm_rep1_rep2) {
                    foreach my $trait_name (@sorted_trait_names) {
                        my $val1 = $result_blup_data_s_prm_rep1->{$stock_name}->{$trait_name} || 'NA';
                        my $val2 = $result_blup_data_s_prm_rep2->{$stock_name}->{$trait_name} || 'NA';
                        print $F_prm_gcorr "$stock_name,$val1,$val2\n";
                    }
                }
            close($F_prm_gcorr);
            ...
            my $result_blup_data_s_grm_prm_sec_rep1;
            my $genetic_effect_min_s_grm_prm_sec_rep1 = 10000000000;
            my $genetic_effect_max_s_grm_prm_sec_rep1 = -10000000000;
            my $genetic_effect_sum_s_grm_prm_sec_rep1 = 0;
            my $genetic_effect_sum_square_s_grm_prm_sec_rep1 = 0;
            my $result_residual_data_s_grm_prm_sec_rep1;
            my $residual_sum_s_grm_prm_sec_rep1 = 0;
            my $residual_sum_square_s_grm_prm_sec_rep1 = 0;
            my $result_fitted_data_s_grm_prm_sec_rep1;
            my $model_sum_square_residual_s_grm_prm_sec_rep1 = 0;
            my @varcomp_original_grm_fixed_effect_grm_prm_sec_rep1;
            my @varcomp_h_grm_fixed_effect_grm_prm_sec_rep1;
            my @fits_grm_fixed_effect_grm_prm_sec_rep1;
            my %seen_accession_names_grm_prm_sec_rep1;

            my $prm_fixed_effects_grm_prm_sec_rep1_cmd = 'R -e "library(sommer); library(data.table); library(reshape2); library(ggplot2); library(GGally);
            mat <- data.frame(fread(\''.$stats_tempfile.'\', header=TRUE, sep=\',\'));
            geno_mat_3col <- data.frame(fread(\''.$grm_file.'\', header=FALSE, sep=\'\t\'));
            prm_mat_cols <- data.frame(fread(\''.$analytics_protocol_data_tempfile28.'\', header=FALSE, sep=\',\'));
            geno_mat <- acast(geno_mat_3col, V1~V2, value.var=\'V3\');
            ...
            "';
            print STDERR Dumper $prm_fixed_effects_grm_prm_sec_rep1_cmd;
            my $prm_fixed_effects_grm_prm_sec_rep1_cmd_status = system($prm_fixed_effects_grm_prm_sec_rep1_cmd);
                ...
                while (my $row = <$fh>) {
                    ...
                    foreach my $encoded_trait (@header_cols_no_prm) {
                        if ($encoded_trait eq $trait_name_encoded_string) {
                            ...
                            my $value = $columns[$col_counter+1];
                            if (defined $value && $value ne '') {
                                $result_blup_data_s_grm_prm_sec_rep1->{$stock_name}->{$trait} = $value;
                                $seen_accession_names_grm_prm_sec_rep1{$stock_name}++;

                                if ($value < $genetic_effect_min_s_grm_prm_sec_rep1) {
                                    $genetic_effect_min_s_grm_prm_sec_rep1 = $value;
                                }
                                elsif ($value >= $genetic_effect_max_s_grm_prm_sec_rep1) {
                                    $genetic_effect_max_s_grm_prm_sec_rep1 = $value;
                                }

                                $genetic_effect_sum_s_grm_prm_sec_rep1 += abs($value);
                                $genetic_effect_sum_square_s_grm_prm_sec_rep1 = $genetic_effect_sum_square_s_grm_prm_sec_rep1 + $value*$value;
                            }
                        }
                        $col_counter++;
                    }
            ...
            open($fh_residual, '<', $stats_out_tempfile_residual) or die "Could not open file '$stats_out_tempfile_residual' $!";
                ...
                while (my $row = <$fh_residual>) {
                    ...
                    my $stock_name = $plot_id_map{$stock_id};
                    if (defined $residual && $residual ne '') {
                        $result_residual_data_s_grm_prm_sec_rep1->{$stock_name}->{$trait_name} = $residual;
                        $residual_sum_s_grm_prm_sec_rep1 += abs($residual);
                        $residual_sum_square_s_grm_prm_sec_rep1 = $residual_sum_square_s_grm_prm_sec_rep1 + $residual*$residual;
                    }
                    if (defined $fitted && $fitted ne '') {
                        $result_fitted_data_s_grm_prm_sec_rep1->{$stock_name}->{$trait_name} = $fitted;
                    }
                    $model_sum_square_residual_s_grm_prm_sec_rep1 = $model_sum_square_residual_s_grm_prm_sec_rep1 + $residual*$residual;
                }
            close($fh_residual);
            ...
            open($fh_varcomp, '<', $stats_out_tempfile_varcomp) or die "Could not open file '$stats_out_tempfile_varcomp' $!";
                ...
                while (my $row = <$fh_varcomp>) {
                    ...
                    push @varcomp_original_grm_fixed_effect_grm_prm_sec_rep1, \@columns;
                }
            close($fh_varcomp);
            ...
            open($fh_varcomp_h, '<', $stats_out_tempfile_vpredict) or die "Could not open file '$stats_out_tempfile_vpredict' $!";
                ...
                while (my $row = <$fh_varcomp_h>) {
                    ...
                    push @varcomp_h_grm_fixed_effect_grm_prm_sec_rep1, \@columns;
                }
            close($fh_varcomp_h);
            ...
            open($fh_fits, '<', $stats_out_tempfile_fits) or die "Could not open file '$stats_out_tempfile_fits' $!";
                ...
                while (my $row = <$fh_fits>) {
                    ...
                    push @fits_grm_fixed_effect_grm_prm_sec_rep1, \@columns;
                }
            close($fh_fits);

            my $result_blup_data_s_grm_prm_sec_rep2;
            my $genetic_effect_min_s_grm_prm_sec_rep2 = 10000000000;
            my $genetic_effect_max_s_grm_prm_sec_rep2 = -10000000000;
            my $genetic_effect_sum_s_grm_prm_sec_rep2 = 0;
            my $genetic_effect_sum_square_s_grm_prm_sec_rep2 = 0;
            my $result_residual_data_s_grm_prm_sec_rep2;
            my $residual_sum_s_grm_prm_sec_rep2 = 0;
            my $residual_sum_square_s_grm_prm_sec_rep2 = 0;
            my $result_fitted_data_s_grm_prm_sec_rep2;
            my $model_sum_square_residual_s_grm_prm_sec_rep2 = 0;
            my @varcomp_original_grm_fixed_effect_grm_prm_sec_rep2;
            my @varcomp_h_grm_fixed_effect_grm_prm_sec_rep2;
            my @fits_grm_fixed_effect_grm_prm_sec_rep2;
            my %seen_accession_names_grm_prm_sec_rep2;

            my $prm_fixed_effects_grm_prm_sec_rep2_cmd = 'R -e "library(sommer); library(data.table); library(reshape2); library(ggplot2); library(GGally);
            mat <- data.frame(fread(\''.$stats_tempfile.'\', header=TRUE, sep=\',\'));
            geno_mat_3col <- data.frame(fread(\''.$grm_file.'\', header=FALSE, sep=\'\t\'));
            prm_mat_cols <- data.frame(fread(\''.$analytics_protocol_data_tempfile28.'\', header=FALSE, sep=\',\'));
            geno_mat <- acast(geno_mat_3col, V1~V2, value.var=\'V3\');
            ...
            "';
            print STDERR Dumper $prm_fixed_effects_grm_prm_sec_rep2_cmd;
            my $prm_fixed_effects_grm_prm_sec_rep2_cmd_status = system($prm_fixed_effects_grm_prm_sec_rep2_cmd);
                ...
                while (my $row = <$fh>) {
                    ...
                    foreach my $encoded_trait (@header_cols_no_prm) {
                        if ($encoded_trait eq $trait_name_encoded_string) {
                            ...
                            my $value = $columns[$col_counter+1];
                            if (defined $value && $value ne '') {
                                $result_blup_data_s_grm_prm_sec_rep2->{$stock_name}->{$trait} = $value;
                                $seen_accession_names_grm_prm_sec_rep2{$stock_name}++;

                                if ($value < $genetic_effect_min_s_grm_prm_sec_rep2) {
                                    $genetic_effect_min_s_grm_prm_sec_rep2 = $value;
                                }
                                elsif ($value >= $genetic_effect_max_s_grm_prm_sec_rep2) {
                                    $genetic_effect_max_s_grm_prm_sec_rep2 = $value;
                                }

                                $genetic_effect_sum_s_grm_prm_sec_rep2 += abs($value);
                                $genetic_effect_sum_square_s_grm_prm_sec_rep2 = $genetic_effect_sum_square_s_grm_prm_sec_rep2 + $value*$value;
                            }
                        }
                        $col_counter++;
                    }
            ...
            open($fh_residual, '<', $stats_out_tempfile_residual) or die "Could not open file '$stats_out_tempfile_residual' $!";
                ...
                while (my $row = <$fh_residual>) {
                    ...
                    my $stock_name = $plot_id_map{$stock_id};
                    if (defined $residual && $residual ne '') {
                        $result_residual_data_s_grm_prm_sec_rep2->{$stock_name}->{$trait_name} = $residual;
                        $residual_sum_s_grm_prm_sec_rep2 += abs($residual);
                        $residual_sum_square_s_grm_prm_sec_rep2 = $residual_sum_square_s_grm_prm_sec_rep2 + $residual*$residual;
                    }
                    if (defined $fitted && $fitted ne '') {
                        $result_fitted_data_s_grm_prm_sec_rep2->{$stock_name}->{$trait_name} = $fitted;
                    }
                    $model_sum_square_residual_s_grm_prm_sec_rep2 = $model_sum_square_residual_s_grm_prm_sec_rep2 + $residual*$residual;
                }
            close($fh_residual);
            ...
            open($fh_varcomp, '<', $stats_out_tempfile_varcomp) or die "Could not open file '$stats_out_tempfile_varcomp' $!";
                ...
                while (my $row = <$fh_varcomp>) {
                    ...
                    push @varcomp_original_grm_fixed_effect_grm_prm_sec_rep2, \@columns;
                }
            close($fh_varcomp);
            ...
            open($fh_varcomp_h, '<', $stats_out_tempfile_vpredict) or die "Could not open file '$stats_out_tempfile_vpredict' $!";
                ...
                while (my $row = <$fh_varcomp_h>) {
                    ...
                    push @varcomp_h_grm_fixed_effect_grm_prm_sec_rep2, \@columns;
                }
            close($fh_varcomp_h);
            ...
            open($fh_fits, '<', $stats_out_tempfile_fits) or die "Could not open file '$stats_out_tempfile_fits' $!";
                ...
                while (my $row = <$fh_fits>) {
                    ...
                    push @fits_grm_fixed_effect_grm_prm_sec_rep2, \@columns;
                }
            close($fh_fits);

            my @seen_accession_names_grm_prm_sec_rep1_rep2;
            foreach my $a (sort keys %seen_accession_names_grm_prm_sec_rep1) {
                if (exists($seen_accession_names_grm_prm_sec_rep2{$a})) {
                    push @seen_accession_names_grm_prm_sec_rep1_rep2, $a;
                }
            }

            open(my $F_grm_prm_sec_gcorr, ">", $analytics_protocol_data_tempfile_grm_prm_secondary_gcorr) || die "Can't open file ".$analytics_protocol_data_tempfile_grm_prm_secondary_gcorr;
                print $F_grm_prm_sec_gcorr "accession_name,g_rep1,g_rep2\n";

                foreach my $stock_name (@seen_accession_names_grm_prm_sec_rep1_rep2) {
                    foreach my $trait_name (@sorted_trait_names) {
                        my $val1 = $result_blup_data_s_grm_prm_sec_rep1->{$stock_name}->{$trait_name} || 'NA';
                        my $val2 = $result_blup_data_s_grm_prm_sec_rep2->{$stock_name}->{$trait_name} || 'NA';
                        print $F_grm_prm_sec_gcorr "$stock_name,$val1,$val2\n";
                    }
                }
            close($F_grm_prm_sec_gcorr);
            ...
            my $result_blup_data_s_grm_prm_rep1;
            my $genetic_effect_min_s_grm_prm_rep1 = 10000000000;
            my $genetic_effect_max_s_grm_prm_rep1 = -10000000000;
            my $genetic_effect_sum_s_grm_prm_rep1 = 0;
            my $genetic_effect_sum_square_s_grm_prm_rep1 = 0;
            my $result_residual_data_s_grm_prm_rep1;
            my $residual_sum_s_grm_prm_rep1 = 0;
            my $residual_sum_square_s_grm_prm_rep1 = 0;
            my $result_fitted_data_s_grm_prm_rep1;
            my $model_sum_square_residual_s_grm_prm_rep1 = 0;
            my @varcomp_original_grm_fixed_effect_grm_prm_rep1;
            my @varcomp_h_grm_fixed_effect_grm_prm_rep1;
            my @fits_grm_fixed_effect_grm_prm_rep1;
            my %seen_accession_names_grm_prm_rep1;

            my $prm_fixed_effects_grm_prm_rep1_cmd = 'R -e "library(sommer); library(data.table); library(reshape2); library(ggplot2); library(GGally);
            mat <- data.frame(fread(\''.$stats_tempfile.'\', header=TRUE, sep=\',\'));
            geno_mat_3col <- data.frame(fread(\''.$grm_file.'\', header=FALSE, sep=\'\t\'));
            prm_mat_cols <- data.frame(fread(\''.$analytics_protocol_data_tempfile27.'\', header=FALSE, sep=\',\'));
            geno_mat <- acast(geno_mat_3col, V1~V2, value.var=\'V3\');
            ...
            "';
            print STDERR Dumper $prm_fixed_effects_grm_prm_rep1_cmd;
            my $prm_fixed_effects_grm_prm_rep1_cmd_status = system($prm_fixed_effects_grm_prm_rep1_cmd);
                ...
                while (my $row = <$fh>) {
                    ...
                    foreach my $encoded_trait (@header_cols_no_prm) {
                        if ($encoded_trait eq $trait_name_encoded_string) {
                            ...
                            my $value = $columns[$col_counter+1];
                            if (defined $value && $value ne '') {
                                $result_blup_data_s_grm_prm_rep1->{$stock_name}->{$trait} = $value;
                                $seen_accession_names_grm_prm_rep1{$stock_name}++;

                                if ($value < $genetic_effect_min_s_grm_prm_rep1) {
                                    $genetic_effect_min_s_grm_prm_rep1 = $value;
                                }
                                elsif ($value >= $genetic_effect_max_s_grm_prm_rep1) {
                                    $genetic_effect_max_s_grm_prm_rep1 = $value;
                                }

                                $genetic_effect_sum_s_grm_prm_rep1 += abs($value);
                                $genetic_effect_sum_square_s_grm_prm_rep1 = $genetic_effect_sum_square_s_grm_prm_rep1 + $value*$value;
                            }
                        }
                        $col_counter++;
                    }
            ...
            open($fh_residual, '<', $stats_out_tempfile_residual) or die "Could not open file '$stats_out_tempfile_residual' $!";
                ...
                while (my $row = <$fh_residual>) {
                    ...
                    my $stock_name = $plot_id_map{$stock_id};
                    if (defined $residual && $residual ne '') {
                        $result_residual_data_s_grm_prm_rep1->{$stock_name}->{$trait_name} = $residual;
                        $residual_sum_s_grm_prm_rep1 += abs($residual);
                        $residual_sum_square_s_grm_prm_rep1 = $residual_sum_square_s_grm_prm_rep1 + $residual*$residual;
                    }
                    if (defined $fitted && $fitted ne '') {
                        $result_fitted_data_s_grm_prm_rep1->{$stock_name}->{$trait_name} = $fitted;
                    }
                    $model_sum_square_residual_s_grm_prm_rep1 = $model_sum_square_residual_s_grm_prm_rep1 + $residual*$residual;
                }
            close($fh_residual);
            ...
            open($fh_varcomp, '<', $stats_out_tempfile_varcomp) or die "Could not open file '$stats_out_tempfile_varcomp' $!";
                ...
                while (my $row = <$fh_varcomp>) {
                    ...
                    push @varcomp_original_grm_fixed_effect_grm_prm_rep1, \@columns;
                }
            close($fh_varcomp);
            ...
            open($fh_varcomp_h, '<', $stats_out_tempfile_vpredict) or die "Could not open file '$stats_out_tempfile_vpredict' $!";
                ...
                while (my $row = <$fh_varcomp_h>) {
                    ...
                    push @varcomp_h_grm_fixed_effect_grm_prm_rep1, \@columns;
                }
            close($fh_varcomp_h);
            ...
            open($fh_fits, '<', $stats_out_tempfile_fits) or die "Could not open file '$stats_out_tempfile_fits' $!";
                ...
                while (my $row = <$fh_fits>) {
                    ...
                    push @fits_grm_fixed_effect_grm_prm_rep1, \@columns;
                }
            close($fh_fits);

            my $result_blup_data_s_grm_prm_rep2;
            my $genetic_effect_min_s_grm_prm_rep2 = 10000000000;
            my $genetic_effect_max_s_grm_prm_rep2 = -10000000000;
            my $genetic_effect_sum_s_grm_prm_rep2 = 0;
            my $genetic_effect_sum_square_s_grm_prm_rep2 = 0;
            my $result_residual_data_s_grm_prm_rep2;
            my $residual_sum_s_grm_prm_rep2 = 0;
            my $residual_sum_square_s_grm_prm_rep2 = 0;
            my $result_fitted_data_s_grm_prm_rep2;
            my $model_sum_square_residual_s_grm_prm_rep2 = 0;
            my @varcomp_original_grm_fixed_effect_grm_prm_rep2;
            my @varcomp_h_grm_fixed_effect_grm_prm_rep2;
            my @fits_grm_fixed_effect_grm_prm_rep2;
            my %seen_accession_names_grm_prm_rep2;

            my $prm_fixed_effects_grm_prm_rep2_cmd = 'R -e "library(sommer); library(data.table); library(reshape2); library(ggplot2); library(GGally);
            mat <- data.frame(fread(\''.$stats_tempfile.'\', header=TRUE, sep=\',\'));
            geno_mat_3col <- data.frame(fread(\''.$grm_file.'\', header=FALSE, sep=\'\t\'));
            prm_mat_cols <- data.frame(fread(\''.$analytics_protocol_data_tempfile27.'\', header=FALSE, sep=\',\'));
            geno_mat <- acast(geno_mat_3col, V1~V2, value.var=\'V3\');
            ...
            "';
            print STDERR Dumper $prm_fixed_effects_grm_prm_rep2_cmd;
            my $prm_fixed_effects_grm_prm_rep2_cmd_status = system($prm_fixed_effects_grm_prm_rep2_cmd);
                ...
                while (my $row = <$fh>) {
                    ...
                    foreach my $encoded_trait (@header_cols_no_prm) {
                        if ($encoded_trait eq $trait_name_encoded_string) {
                            ...
                            my $value = $columns[$col_counter+1];
                            if (defined $value && $value ne '') {
                                $result_blup_data_s_grm_prm_rep2->{$stock_name}->{$trait} = $value;
                                $seen_accession_names_grm_prm_rep2{$stock_name}++;

                                if ($value < $genetic_effect_min_s_grm_prm_rep2) {
                                    $genetic_effect_min_s_grm_prm_rep2 = $value;
                                }
                                elsif ($value >= $genetic_effect_max_s_grm_prm_rep2) {
                                    $genetic_effect_max_s_grm_prm_rep2 = $value;
                                }

                                $genetic_effect_sum_s_grm_prm_rep2 += abs($value);
                                $genetic_effect_sum_square_s_grm_prm_rep2 = $genetic_effect_sum_square_s_grm_prm_rep2 + $value*$value;
                            }
                        }
                        $col_counter++;
                    }
            ...
            open($fh_residual, '<', $stats_out_tempfile_residual) or die "Could not open file '$stats_out_tempfile_residual' $!";
                ...
                while (my $row = <$fh_residual>) {
                    ...
                    my $stock_name = $plot_id_map{$stock_id};
                    if (defined $residual && $residual ne '') {
                        $result_residual_data_s_grm_prm_rep2->{$stock_name}->{$trait_name} = $residual;
                        $residual_sum_s_grm_prm_rep2 += abs($residual);
                        $residual_sum_square_s_grm_prm_rep2 = $residual_sum_square_s_grm_prm_rep2 + $residual*$residual;
                    }
                    if (defined $fitted && $fitted ne '') {
                        $result_fitted_data_s_grm_prm_rep2->{$stock_name}->{$trait_name} = $fitted;
                    }
                    $model_sum_square_residual_s_grm_prm_rep2 = $model_sum_square_residual_s_grm_prm_rep2 + $residual*$residual;
                }
            close($fh_residual);
            ...
            open($fh_varcomp, '<', $stats_out_tempfile_varcomp) or die "Could not open file '$stats_out_tempfile_varcomp' $!";
                ...
                while (my $row = <$fh_varcomp>) {
                    ...
                    push @varcomp_original_grm_fixed_effect_grm_prm_rep2, \@columns;
                }
            close($fh_varcomp);
            ...
            open($fh_varcomp_h, '<', $stats_out_tempfile_vpredict) or die "Could not open file '$stats_out_tempfile_vpredict' $!";
                ...
                while (my $row = <$fh_varcomp_h>) {
                    ...
                    push @varcomp_h_grm_fixed_effect_grm_prm_rep2, \@columns;
                }
            close($fh_varcomp_h);
            ...
            open($fh_fits, '<', $stats_out_tempfile_fits) or die "Could not open file '$stats_out_tempfile_fits' $!";
                ...
                while (my $row = <$fh_fits>) {
                    ...
                    push @fits_grm_fixed_effect_grm_prm_rep2, \@columns;
                }
            close($fh_fits);

            my @seen_accession_names_grm_prm_rep1_rep2;
            foreach my $a (sort keys %seen_accession_names_grm_prm_rep1) {
                if (exists($seen_accession_names_grm_prm_rep2{$a})) {
                    push @seen_accession_names_grm_prm_rep1_rep2, $a;
                }
            }

            open(my $F_grm_prm_gcorr, ">", $analytics_protocol_data_tempfile_grm_prm_gcorr) || die "Can't open file ".$analytics_protocol_data_tempfile_grm_prm_gcorr;
                print $F_grm_prm_gcorr "accession_name,g_rep1,g_rep2\n";

                foreach my $stock_name (@seen_accession_names_grm_prm_rep1_rep2) {
                    foreach my $trait_name (@sorted_trait_names) {
                        my $val1 = $result_blup_data_s_grm_prm_rep1->{$stock_name}->{$trait_name} || 'NA';
                        my $val2 = $result_blup_data_s_grm_prm_rep2->{$stock_name}->{$trait_name} || 'NA';
                        print $F_grm_prm_gcorr "$stock_name,$val1,$val2\n";
                    }
                }
            close($F_grm_prm_gcorr);
            ...
            my $result_blup_data_s_grm_id_rep1;
            my $genetic_effect_min_s_grm_id_rep1 = 10000000000;
            my $genetic_effect_max_s_grm_id_rep1 = -10000000000;
            my $genetic_effect_sum_s_grm_id_rep1 = 0;
            my $genetic_effect_sum_square_s_grm_id_rep1 = 0;
            my $result_residual_data_s_grm_id_rep1;
            my $residual_sum_s_grm_id_rep1 = 0;
            my $residual_sum_square_s_grm_id_rep1 = 0;
            my $result_fitted_data_s_grm_id_rep1;
            my $model_sum_square_residual_s_grm_id_rep1 = 0;
            my @varcomp_original_grm_fixed_effect_grm_id_rep1;
            my @varcomp_h_grm_fixed_effect_grm_id_rep1;
            my @fits_grm_fixed_effect_grm_id_rep1;
            my %seen_accession_names_grm_id_rep1;

            my $prm_fixed_effects_grm_id_rep1_cmd = 'R -e "library(sommer); library(data.table); library(reshape2); library(ggplot2); library(GGally);
            mat <- data.frame(fread(\''.$stats_tempfile.'\', header=TRUE, sep=\',\'));
            geno_mat_3col <- data.frame(fread(\''.$grm_file.'\', header=FALSE, sep=\'\t\'));
            prm_mat_cols <- data.frame(fread(\''.$analytics_protocol_data_tempfile27.'\', header=FALSE, sep=\',\'));
            geno_mat <- acast(geno_mat_3col, V1~V2, value.var=\'V3\');
            ...
            "';
            print STDERR Dumper $prm_fixed_effects_grm_id_rep1_cmd;
            my $prm_fixed_effects_grm_id_rep1_cmd_status = system($prm_fixed_effects_grm_id_rep1_cmd);
                ...
                while (my $row = <$fh>) {
                    ...
                    foreach my $encoded_trait (@header_cols_no_prm) {
                        if ($encoded_trait eq $trait_name_encoded_string) {
                            ...
                            my $value = $columns[$col_counter+1];
                            if (defined $value && $value ne '') {
                                $result_blup_data_s_grm_id_rep1->{$stock_name}->{$trait} = $value;
                                $seen_accession_names_grm_id_rep1{$stock_name}++;

                                if ($value < $genetic_effect_min_s_grm_id_rep1) {
                                    $genetic_effect_min_s_grm_id_rep1 = $value;
                                }
                                elsif ($value >= $genetic_effect_max_s_grm_id_rep1) {
                                    $genetic_effect_max_s_grm_id_rep1 = $value;
                                }

                                $genetic_effect_sum_s_grm_id_rep1 += abs($value);
                                $genetic_effect_sum_square_s_grm_id_rep1 = $genetic_effect_sum_square_s_grm_id_rep1 + $value*$value;
                            }
                        }
                        $col_counter++;
                    }
            ...
            open($fh_residual, '<', $stats_out_tempfile_residual) or die "Could not open file '$stats_out_tempfile_residual' $!";
                ...
                while (my $row = <$fh_residual>) {
                    ...
                    my $stock_name = $plot_id_map{$stock_id};
                    if (defined $residual && $residual ne '') {
                        $result_residual_data_s_grm_id_rep1->{$stock_name}->{$trait_name} = $residual;
                        $residual_sum_s_grm_id_rep1 += abs($residual);
                        $residual_sum_square_s_grm_id_rep1 = $residual_sum_square_s_grm_id_rep1 + $residual*$residual;
                    }
                    if (defined $fitted && $fitted ne '') {
                        $result_fitted_data_s_grm_id_rep1->{$stock_name}->{$trait_name} = $fitted;
                    }
                    $model_sum_square_residual_s_grm_id_rep1 = $model_sum_square_residual_s_grm_id_rep1 + $residual*$residual;
                }
            close($fh_residual);
            ...
            open($fh_varcomp, '<', $stats_out_tempfile_varcomp) or die "Could not open file '$stats_out_tempfile_varcomp' $!";
                ...
                while (my $row = <$fh_varcomp>) {
                    ...
                    push @varcomp_original_grm_fixed_effect_grm_id_rep1, \@columns;
                }
            close($fh_varcomp);
            ...
            open($fh_varcomp_h, '<', $stats_out_tempfile_vpredict) or die "Could not open file '$stats_out_tempfile_vpredict' $!";
                ...
                while (my $row = <$fh_varcomp_h>) {
                    ...
                    push @varcomp_h_grm_fixed_effect_grm_id_rep1, \@columns;
                }
            close($fh_varcomp_h);
            ...
            open($fh_fits, '<', $stats_out_tempfile_fits) or die "Could not open file '$stats_out_tempfile_fits' $!";
                ...
                while (my $row = <$fh_fits>) {
                    ...
                    push @fits_grm_fixed_effect_grm_id_rep1, \@columns;
                }
            close($fh_fits);

            my $result_blup_data_s_grm_id_rep2;
            my $genetic_effect_min_s_grm_id_rep2 = 10000000000;
            my $genetic_effect_max_s_grm_id_rep2 = -10000000000;
            my $genetic_effect_sum_s_grm_id_rep2 = 0;
            my $genetic_effect_sum_square_s_grm_id_rep2 = 0;
            my $result_residual_data_s_grm_id_rep2;
            my $residual_sum_s_grm_id_rep2 = 0;
            my $residual_sum_square_s_grm_id_rep2 = 0;
            my $result_fitted_data_s_grm_id_rep2;
            my $model_sum_square_residual_s_grm_id_rep2 = 0;
            my @varcomp_original_grm_fixed_effect_grm_id_rep2;
            my @varcomp_h_grm_fixed_effect_grm_id_rep2;
            my @fits_grm_fixed_effect_grm_id_rep2;
            my %seen_accession_names_grm_id_rep2;

            my $prm_fixed_effects_grm_id_rep2_cmd = 'R -e "library(sommer); library(data.table); library(reshape2); library(ggplot2); library(GGally);
            mat <- data.frame(fread(\''.$stats_tempfile.'\', header=TRUE, sep=\',\'));
            geno_mat_3col <- data.frame(fread(\''.$grm_file.'\', header=FALSE, sep=\'\t\'));
            prm_mat_cols <- data.frame(fread(\''.$analytics_protocol_data_tempfile27.'\', header=FALSE, sep=\',\'));
            geno_mat <- acast(geno_mat_3col, V1~V2, value.var=\'V3\');
            ...
            "';
            print STDERR Dumper $prm_fixed_effects_grm_id_rep2_cmd;
            my $prm_fixed_effects_grm_id_rep2_cmd_status = system($prm_fixed_effects_grm_id_rep2_cmd);
                ...
                while (my $row = <$fh>) {
                    ...
                    foreach my $encoded_trait (@header_cols_no_prm) {
                        if ($encoded_trait eq $trait_name_encoded_string) {
                            ...
                            my $value = $columns[$col_counter+1];
                            if (defined $value && $value ne '') {
                                $result_blup_data_s_grm_id_rep2->{$stock_name}->{$trait} = $value;
                                $seen_accession_names_grm_id_rep2{$stock_name}++;

                                if ($value < $genetic_effect_min_s_grm_id_rep2) {
                                    $genetic_effect_min_s_grm_id_rep2 = $value;
                                }
                                elsif ($value >= $genetic_effect_max_s_grm_id_rep2) {
                                    $genetic_effect_max_s_grm_id_rep2 = $value;
                                }

                                $genetic_effect_sum_s_grm_id_rep2 += abs($value);
                                $genetic_effect_sum_square_s_grm_id_rep2 = $genetic_effect_sum_square_s_grm_id_rep2 + $value*$value;
                            }
                        }
                        $col_counter++;
                    }
            ...
            open($fh_residual, '<', $stats_out_tempfile_residual) or die "Could not open file '$stats_out_tempfile_residual' $!";
                ...
                while (my $row = <$fh_residual>) {
                    ...
                    my $stock_name = $plot_id_map{$stock_id};
                    if (defined $residual && $residual ne '') {
                        $result_residual_data_s_grm_id_rep2->{$stock_name}->{$trait_name} = $residual;
                        $residual_sum_s_grm_id_rep2 += abs($residual);
                        $residual_sum_square_s_grm_id_rep2 = $residual_sum_square_s_grm_id_rep2 + $residual*$residual;
                    }
                    if (defined $fitted && $fitted ne '') {
                        $result_fitted_data_s_grm_id_rep2->{$stock_name}->{$trait_name} = $fitted;
                    }
                    $model_sum_square_residual_s_grm_id_rep2 = $model_sum_square_residual_s_grm_id_rep2 + $residual*$residual;
                }
            close($fh_residual);
            ...
            open($fh_varcomp, '<', $stats_out_tempfile_varcomp) or die "Could not open file '$stats_out_tempfile_varcomp' $!";
                ...
                while (my $row = <$fh_varcomp>) {
                    ...
                    push @varcomp_original_grm_fixed_effect_grm_id_rep2, \@columns;
                }
            close($fh_varcomp);
            ...
            open($fh_varcomp_h, '<', $stats_out_tempfile_vpredict) or die "Could not open file '$stats_out_tempfile_vpredict' $!";
                ...
                while (my $row = <$fh_varcomp_h>) {
                    ...
                    push @varcomp_h_grm_fixed_effect_grm_id_rep2, \@columns;
                }
            close($fh_varcomp_h);
            ...
            open($fh_fits, '<', $stats_out_tempfile_fits) or die "Could not open file '$stats_out_tempfile_fits' $!";
                ...
                while (my $row = <$fh_fits>) {
                    ...
                    push @fits_grm_fixed_effect_grm_id_rep2, \@columns;
                }
            close($fh_fits);

            my @seen_accession_names_grm_id_rep1_rep2;
            foreach my $a (sort keys %seen_accession_names_grm_id_rep1) {
                if (exists($seen_accession_names_grm_id_rep2{$a})) {
                    push @seen_accession_names_grm_id_rep1_rep2, $a;
                }
            }

            open(my $F_grm_id_gcorr, ">", $analytics_protocol_data_tempfile_grm_id_gcorr) || die "Can't open file ".$analytics_protocol_data_tempfile_grm_id_gcorr;
                print $F_grm_id_gcorr "accession_name,g_rep1,g_rep2\n";

                foreach my $stock_name (@seen_accession_names_grm_id_rep1_rep2) {
                    foreach my $trait_name (@sorted_trait_names) {
                        my $val1 = $result_blup_data_s_grm_id_rep1->{$stock_name}->{$trait_name} || 'NA';
                        my $val2 = $result_blup_data_s_grm_id_rep2->{$stock_name}->{$trait_name} || 'NA';
                        print $F_grm_id_gcorr "$stock_name,$val1,$val2\n";
                    }
                }
            close($F_grm_id_gcorr);
            ...
            my $result_blup_data_s_grm_id_prm_rep1;
            my $genetic_effect_min_s_grm_id_prm_rep1 = 10000000000;
            my $genetic_effect_max_s_grm_id_prm_rep1 = -10000000000;
            my $genetic_effect_sum_s_grm_id_prm_rep1 = 0;
            my $genetic_effect_sum_square_s_grm_id_prm_rep1 = 0;
            my $result_residual_data_s_grm_id_prm_rep1;
            my $residual_sum_s_grm_id_prm_rep1 = 0;
            my $residual_sum_square_s_grm_id_prm_rep1 = 0;
            my $result_fitted_data_s_grm_id_prm_rep1;
            my $model_sum_square_residual_s_grm_id_prm_rep1 = 0;
            my @varcomp_original_grm_fixed_effect_grm_id_prm_rep1;
            my @varcomp_h_grm_fixed_effect_grm_id_prm_rep1;
            my @fits_grm_fixed_effect_grm_id_prm_rep1;
            my %seen_accession_names_grm_id_prm_rep1;

            my $prm_fixed_effects_grm_id_prm_rep1_cmd = 'R -e "library(sommer); library(data.table); library(reshape2); library(ggplot2); library(GGally);
            mat <- data.frame(fread(\''.$stats_tempfile.'\', header=TRUE, sep=\',\'));
            geno_mat_3col <- data.frame(fread(\''.$grm_file.'\', header=FALSE, sep=\'\t\'));
            prm_mat_cols <- data.frame(fread(\''.$analytics_protocol_data_tempfile27.'\', header=FALSE, sep=\',\'));
            geno_mat <- acast(geno_mat_3col, V1~V2, value.var=\'V3\');
            ...
            "';
            print STDERR Dumper $prm_fixed_effects_grm_id_prm_rep1_cmd;
            my $prm_fixed_effects_grm_id_prm_rep1_cmd_status = system($prm_fixed_effects_grm_id_prm_rep1_cmd);
                ...
                while (my $row = <$fh>) {
                    ...
                    foreach my $encoded_trait (@header_cols_no_prm) {
                        if ($encoded_trait eq $trait_name_encoded_string) {
                            ...
                            my $value = $columns[$col_counter+1];
                            if (defined $value && $value ne '') {
                                $result_blup_data_s_grm_id_prm_rep1->{$stock_name}->{$trait} = $value;
                                $seen_accession_names_grm_id_prm_rep1{$stock_name}++;

                                if ($value < $genetic_effect_min_s_grm_id_prm_rep1) {
                                    $genetic_effect_min_s_grm_id_prm_rep1 = $value;
                                }
                                elsif ($value >= $genetic_effect_max_s_grm_id_prm_rep1) {
                                    $genetic_effect_max_s_grm_id_prm_rep1 = $value;
                                }

                                $genetic_effect_sum_s_grm_id_prm_rep1 += abs($value);
                                $genetic_effect_sum_square_s_grm_id_prm_rep1 = $genetic_effect_sum_square_s_grm_id_prm_rep1 + $value*$value;
                            }
                        }
                        $col_counter++;
                    }
            ...
            open($fh_residual, '<', $stats_out_tempfile_residual) or die "Could not open file '$stats_out_tempfile_residual' $!";
                ...
                while (my $row = <$fh_residual>) {
                    ...
                    my $stock_name = $plot_id_map{$stock_id};
                    if (defined $residual && $residual ne '') {
                        $result_residual_data_s_grm_id_prm_rep1->{$stock_name}->{$trait_name} = $residual;
                        $residual_sum_s_grm_id_prm_rep1 += abs($residual);
                        $residual_sum_square_s_grm_id_prm_rep1 = $residual_sum_square_s_grm_id_prm_rep1 + $residual*$residual;
                    }
                    if (defined $fitted && $fitted ne '') {
                        $result_fitted_data_s_grm_id_prm_rep1->{$stock_name}->{$trait_name} = $fitted;
                    }
                    $model_sum_square_residual_s_grm_id_prm_rep1 = $model_sum_square_residual_s_grm_id_prm_rep1 + $residual*$residual;
                }
            close($fh_residual);
            ...
            open($fh_varcomp, '<', $stats_out_tempfile_varcomp) or die "Could not open file '$stats_out_tempfile_varcomp' $!";
                ...
                while (my $row = <$fh_varcomp>) {
                    ...
                    push @varcomp_original_grm_fixed_effect_grm_id_prm_rep1, \@columns;
                }
            close($fh_varcomp);
            ...
            open($fh_varcomp_h, '<', $stats_out_tempfile_vpredict) or die "Could not open file '$stats_out_tempfile_vpredict' $!";
                ...
                while (my $row = <$fh_varcomp_h>) {
                    ...
                    push @varcomp_h_grm_fixed_effect_grm_id_prm_rep1, \@columns;
                }
            close($fh_varcomp_h);
            ...
            open($fh_fits, '<', $stats_out_tempfile_fits) or die "Could not open file '$stats_out_tempfile_fits' $!";
                ...
                while (my $row = <$fh_fits>) {
                    ...
                    push @fits_grm_fixed_effect_grm_id_prm_rep1, \@columns;
                }
            close($fh_fits);

            my $result_blup_data_s_grm_id_prm_rep2;
            my $genetic_effect_min_s_grm_id_prm_rep2 = 10000000000;
            my $genetic_effect_max_s_grm_id_prm_rep2 = -10000000000;
            my $genetic_effect_sum_s_grm_id_prm_rep2 = 0;
            my $genetic_effect_sum_square_s_grm_id_prm_rep2 = 0;
            my $result_residual_data_s_grm_id_prm_rep2;
            my $residual_sum_s_grm_id_prm_rep2 = 0;
            my $residual_sum_square_s_grm_id_prm_rep2 = 0;
            my $result_fitted_data_s_grm_id_prm_rep2;
            my $model_sum_square_residual_s_grm_id_prm_rep2 = 0;
            my @varcomp_original_grm_fixed_effect_grm_id_prm_rep2;
            my @varcomp_h_grm_fixed_effect_grm_id_prm_rep2;
            my @fits_grm_fixed_effect_grm_id_prm_rep2;
            my %seen_accession_names_grm_id_prm_rep2;

            my $prm_fixed_effects_grm_id_prm_rep2_cmd = 'R -e "library(sommer); library(data.table); library(reshape2); library(ggplot2); library(GGally);
            mat <- data.frame(fread(\''.$stats_tempfile.'\', header=TRUE, sep=\',\'));
            geno_mat_3col <- data.frame(fread(\''.$grm_file.'\', header=FALSE, sep=\'\t\'));
            prm_mat_cols <- data.frame(fread(\''.$analytics_protocol_data_tempfile27.'\', header=FALSE, sep=\',\'));
            geno_mat <- acast(geno_mat_3col, V1~V2, value.var=\'V3\');
            ...
            "';
            print STDERR Dumper $prm_fixed_effects_grm_id_prm_rep2_cmd;
            my $prm_fixed_effects_grm_id_prm_rep2_cmd_status = system($prm_fixed_effects_grm_id_prm_rep2_cmd);
                ...
                while (my $row = <$fh>) {
                    ...
                    foreach my $encoded_trait (@header_cols_no_prm) {
                        if ($encoded_trait eq $trait_name_encoded_string) {
                            ...
                            my $value = $columns[$col_counter+1];
                            if (defined $value && $value ne '') {
                                $result_blup_data_s_grm_id_prm_rep2->{$stock_name}->{$trait} = $value;
                                $seen_accession_names_grm_id_prm_rep2{$stock_name}++;

                                if ($value < $genetic_effect_min_s_grm_id_prm_rep2) {
                                    $genetic_effect_min_s_grm_id_prm_rep2 = $value;
                                }
                                elsif ($value >= $genetic_effect_max_s_grm_id_prm_rep2) {
                                    $genetic_effect_max_s_grm_id_prm_rep2 = $value;
                                }

                                $genetic_effect_sum_s_grm_id_prm_rep2 += abs($value);
                                $genetic_effect_sum_square_s_grm_id_prm_rep2 = $genetic_effect_sum_square_s_grm_id_prm_rep2 + $value*$value;
                            }
                        }
                        $col_counter++;
                    }
            ...
            open($fh_residual, '<', $stats_out_tempfile_residual) or die "Could not open file '$stats_out_tempfile_residual' $!";
                ...
                while (my $row = <$fh_residual>) {
                    ...
                    my $stock_name = $plot_id_map{$stock_id};
                    if (defined $residual && $residual ne '') {
                        $result_residual_data_s_grm_id_prm_rep2->{$stock_name}->{$trait_name} = $residual;
                        $residual_sum_s_grm_id_prm_rep2 += abs($residual);
                        $residual_sum_square_s_grm_id_prm_rep2 = $residual_sum_square_s_grm_id_prm_rep2 + $residual*$residual;
                    }
                    if (defined $fitted && $fitted ne '') {
                        $result_fitted_data_s_grm_id_prm_rep2->{$stock_name}->{$trait_name} = $fitted;
                    }
                    $model_sum_square_residual_s_grm_id_prm_rep2 = $model_sum_square_residual_s_grm_id_prm_rep2 + $residual*$residual;
                }
            close($fh_residual);
            ...
            open($fh_varcomp, '<', $stats_out_tempfile_varcomp) or die "Could not open file '$stats_out_tempfile_varcomp' $!";
                ...
                while (my $row = <$fh_varcomp>) {
                    ...
                    push @varcomp_original_grm_fixed_effect_grm_id_prm_rep2, \@columns;
                }
            close($fh_varcomp);
            ...
            open($fh_varcomp_h, '<', $stats_out_tempfile_vpredict) or die "Could not open file '$stats_out_tempfile_vpredict' $!";
                ...
                while (my $row = <$fh_varcomp_h>) {
                    ...
                    push @varcomp_h_grm_fixed_effect_grm_id_prm_rep2, \@columns;
                }
            close($fh_varcomp_h);
            ...
            open($fh_fits, '<', $stats_out_tempfile_fits) or die "Could not open file '$stats_out_tempfile_fits' $!";
                ...
                while (my $row = <$fh_fits>) {
                    ...
                    push @fits_grm_fixed_effect_grm_id_prm_rep2, \@columns;
                }
            close($fh_fits);

            my @seen_accession_names_grm_id_prm_rep1_rep2;
            foreach my $a (sort keys %seen_accession_names_grm_id_prm_rep1) {
                if (exists($seen_accession_names_grm_id_prm_rep2{$a})) {
                    push @seen_accession_names_grm_id_prm_rep1_rep2, $a;
                }
            }

            open(my $F_grm_id_prm_gcorr, ">", $analytics_protocol_data_tempfile_grm_id_prm_gcorr) || die "Can't open file ".$analytics_protocol_data_tempfile_grm_id_prm_gcorr;
                print $F_grm_id_prm_gcorr "accession_name,g_rep1,g_rep2\n";

                foreach my $stock_name (@seen_accession_names_grm_id_prm_rep1_rep2) {
                    foreach my $trait_name (@sorted_trait_names) {
                        my $val1 = $result_blup_data_s_grm_id_prm_rep1->{$stock_name}->{$trait_name} || 'NA';
                        my $val2 = $result_blup_data_s_grm_id_prm_rep2->{$stock_name}->{$trait_name} || 'NA';
                        print $F_grm_id_prm_gcorr "$stock_name,$val1,$val2\n";
                    }
                }
            close($F_grm_id_prm_gcorr);
            ...
            my $result_blup_data_s_grm_id_prm_id_rep1;
            my $genetic_effect_min_s_grm_id_prm_id_rep1 = 10000000000;
            my $genetic_effect_max_s_grm_id_prm_id_rep1 = -10000000000;
            my $genetic_effect_sum_s_grm_id_prm_id_rep1 = 0;
            my $genetic_effect_sum_square_s_grm_id_prm_id_rep1 = 0;
            my $result_residual_data_s_grm_id_prm_id_rep1;
            my $residual_sum_s_grm_id_prm_id_rep1 = 0;
            my $residual_sum_square_s_grm_id_prm_id_rep1 = 0;
            my $result_fitted_data_s_grm_id_prm_id_rep1;
            my $model_sum_square_residual_s_grm_id_prm_id_rep1 = 0;
            my @varcomp_original_grm_fixed_effect_grm_id_prm_id_rep1;
            my @varcomp_h_grm_fixed_effect_grm_id_prm_id_rep1;
            my @fits_grm_fixed_effect_grm_id_prm_id_rep1;
            my %seen_accession_names_grm_id_prm_id_rep1;

            my $prm_fixed_effects_grm_id_prm_id_rep1_cmd = 'R -e "library(sommer); library(data.table); library(reshape2); library(ggplot2); library(GGally);
            mat <- data.frame(fread(\''.$stats_tempfile.'\', header=TRUE, sep=\',\'));
            geno_mat_3col <- data.frame(fread(\''.$grm_file.'\', header=FALSE, sep=\'\t\'));
            prm_mat_cols <- data.frame(fread(\''.$analytics_protocol_data_tempfile27.'\', header=FALSE, sep=\',\'));
            geno_mat <- acast(geno_mat_3col, V1~V2, value.var=\'V3\');
            ...
            "';
            print STDERR Dumper $prm_fixed_effects_grm_id_prm_id_rep1_cmd;
            my $prm_fixed_effects_grm_id_prm_id_rep1_cmd_status = system($prm_fixed_effects_grm_id_prm_id_rep1_cmd);
                ...
                while (my $row = <$fh>) {
                    ...
                    foreach my $encoded_trait (@header_cols_no_prm) {
                        if ($encoded_trait eq $trait_name_encoded_string) {
                            ...
                            my $value = $columns[$col_counter+1];
                            if (defined $value && $value ne '') {
                                $result_blup_data_s_grm_id_prm_id_rep1->{$stock_name}->{$trait} = $value;
                                $seen_accession_names_grm_id_prm_id_rep1{$stock_name}++;

                                if ($value < $genetic_effect_min_s_grm_id_prm_id_rep1) {
                                    $genetic_effect_min_s_grm_id_prm_id_rep1 = $value;
                                }
                                elsif ($value >= $genetic_effect_max_s_grm_id_prm_id_rep1) {
                                    $genetic_effect_max_s_grm_id_prm_id_rep1 = $value;
                                }

                                $genetic_effect_sum_s_grm_id_prm_id_rep1 += abs($value);
                                $genetic_effect_sum_square_s_grm_id_prm_id_rep1 = $genetic_effect_sum_square_s_grm_id_prm_id_rep1 + $value*$value;
                            }
                        }
                        $col_counter++;
                    }
            ...
            open($fh_residual, '<', $stats_out_tempfile_residual) or die "Could not open file '$stats_out_tempfile_residual' $!";
                ...
                while (my $row = <$fh_residual>) {
                    ...
                    my $stock_name = $plot_id_map{$stock_id};
                    if (defined $residual && $residual ne '') {
                        $result_residual_data_s_grm_id_prm_id_rep1->{$stock_name}->{$trait_name} = $residual;
                        $residual_sum_s_grm_id_prm_id_rep1 += abs($residual);
                        $residual_sum_square_s_grm_id_prm_id_rep1 = $residual_sum_square_s_grm_id_prm_id_rep1 + $residual*$residual;
                    }
                    if (defined $fitted && $fitted ne '') {
                        $result_fitted_data_s_grm_id_prm_id_rep1->{$stock_name}->{$trait_name} = $fitted;
                    }
                    $model_sum_square_residual_s_grm_id_prm_id_rep1 = $model_sum_square_residual_s_grm_id_prm_id_rep1 + $residual*$residual;
                }
            close($fh_residual);
            ...
            open($fh_varcomp, '<', $stats_out_tempfile_varcomp) or die "Could not open file '$stats_out_tempfile_varcomp' $!";
                ...
                while (my $row = <$fh_varcomp>) {
                    ...
                    push @varcomp_original_grm_fixed_effect_grm_id_prm_id_rep1, \@columns;
                }
            close($fh_varcomp);
            ...
            open($fh_varcomp_h, '<', $stats_out_tempfile_vpredict) or die "Could not open file '$stats_out_tempfile_vpredict' $!";
                ...
                while (my $row = <$fh_varcomp_h>) {
                    ...
                    push @varcomp_h_grm_fixed_effect_grm_id_prm_id_rep1, \@columns;
                }
            close($fh_varcomp_h);
            ...
            open($fh_fits, '<', $stats_out_tempfile_fits) or die "Could not open file '$stats_out_tempfile_fits' $!";
                ...
                while (my $row = <$fh_fits>) {
                    ...
                    push @fits_grm_fixed_effect_grm_id_prm_id_rep1, \@columns;
                }
            close($fh_fits);

            my $result_blup_data_s_grm_id_prm_id_rep2;
            my $genetic_effect_min_s_grm_id_prm_id_rep2 = 10000000000;
            my $genetic_effect_max_s_grm_id_prm_id_rep2 = -10000000000;
            my $genetic_effect_sum_s_grm_id_prm_id_rep2 = 0;
            my $genetic_effect_sum_square_s_grm_id_prm_id_rep2 = 0;
            my $result_residual_data_s_grm_id_prm_id_rep2;
            my $residual_sum_s_grm_id_prm_id_rep2 = 0;
            my $residual_sum_square_s_grm_id_prm_id_rep2 = 0;
            my $result_fitted_data_s_grm_id_prm_id_rep2;
            my $model_sum_square_residual_s_grm_id_prm_id_rep2 = 0;
            my @varcomp_original_grm_fixed_effect_grm_id_prm_id_rep2;
            my @varcomp_h_grm_fixed_effect_grm_id_prm_id_rep2;
            my @fits_grm_fixed_effect_grm_id_prm_id_rep2;
            my %seen_accession_names_grm_id_prm_id_rep2;

            my $prm_fixed_effects_grm_id_prm_id_rep2_cmd = 'R -e "library(sommer); library(data.table); library(reshape2); library(ggplot2); library(GGally);
            mat <- data.frame(fread(\''.$stats_tempfile.'\', header=TRUE, sep=\',\'));
            geno_mat_3col <- data.frame(fread(\''.$grm_file.'\', header=FALSE, sep=\'\t\'));
            prm_mat_cols <- data.frame(fread(\''.$analytics_protocol_data_tempfile27.'\', header=FALSE, sep=\',\'));
            geno_mat <- acast(geno_mat_3col, V1~V2, value.var=\'V3\');
            ...
            "';
            print STDERR Dumper $prm_fixed_effects_grm_id_prm_id_rep2_cmd;
            my $prm_fixed_effects_grm_id_prm_id_rep2_cmd_status = system($prm_fixed_effects_grm_id_prm_id_rep2_cmd);
                ...
                while (my $row = <$fh>) {
                    ...
                    foreach my $encoded_trait (@header_cols_no_prm) {
                        if ($encoded_trait eq $trait_name_encoded_string) {
                            ...
                            my $value = $columns[$col_counter+1];
                            if (defined $value && $value ne '') {
                                $result_blup_data_s_grm_id_prm_id_rep2->{$stock_name}->{$trait} = $value;
                                $seen_accession_names_grm_id_prm_id_rep2{$stock_name}++;

                                if ($value < $genetic_effect_min_s_grm_id_prm_id_rep2) {
                                    $genetic_effect_min_s_grm_id_prm_id_rep2 = $value;
                                }
                                elsif ($value >= $genetic_effect_max_s_grm_id_prm_id_rep2) {
                                    $genetic_effect_max_s_grm_id_prm_id_rep2 = $value;
                                }

                                $genetic_effect_sum_s_grm_id_prm_id_rep2 += abs($value);
                                $genetic_effect_sum_square_s_grm_id_prm_id_rep2 = $genetic_effect_sum_square_s_grm_id_prm_id_rep2 + $value*$value;
                            }
                        }
                        $col_counter++;
                    }
            ...
            open($fh_residual, '<', $stats_out_tempfile_residual) or die "Could not open file '$stats_out_tempfile_residual' $!";
                ...
                while (my $row = <$fh_residual>) {
                    ...
                    my $stock_name = $plot_id_map{$stock_id};
                    if (defined $residual && $residual ne '') {
                        $result_residual_data_s_grm_id_prm_id_rep2->{$stock_name}->{$trait_name} = $residual;
                        $residual_sum_s_grm_id_prm_id_rep2 += abs($residual);
                        $residual_sum_square_s_grm_id_prm_id_rep2 = $residual_sum_square_s_grm_id_prm_id_rep2 + $residual*$residual;
                    }
                    if (defined $fitted && $fitted ne '') {
                        $result_fitted_data_s_grm_id_prm_id_rep2->{$stock_name}->{$trait_name} = $fitted;
                    }
                    $model_sum_square_residual_s_grm_id_prm_id_rep2 = $model_sum_square_residual_s_grm_id_prm_id_rep2 + $residual*$residual;
                }
            close($fh_residual);
            ...
            open($fh_varcomp, '<', $stats_out_tempfile_varcomp) or die "Could not open file '$stats_out_tempfile_varcomp' $!";
                ...
                while (my $row = <$fh_varcomp>) {
                    ...
                    push @varcomp_original_grm_fixed_effect_grm_id_prm_id_rep2, \@columns;
                }
            close($fh_varcomp);
            ...
            open($fh_varcomp_h, '<', $stats_out_tempfile_vpredict) or die "Could not open file '$stats_out_tempfile_vpredict' $!";
                ...
                while (my $row = <$fh_varcomp_h>) {
                    ...
                    push @varcomp_h_grm_fixed_effect_grm_id_prm_id_rep2, \@columns;
                }
            close($fh_varcomp_h);
            ...
            open($fh_fits, '<', $stats_out_tempfile_fits) or die "Could not open file '$stats_out_tempfile_fits' $!";
                ...
                while (my $row = <$fh_fits>) {
                    ...
                    push @fits_grm_fixed_effect_grm_id_prm_id_rep2, \@columns;
                }
            close($fh_fits);

            my @seen_accession_names_grm_id_prm_id_rep1_rep2;
            foreach my $a (sort keys %seen_accession_names_grm_id_prm_id_rep1) {
                if (exists($seen_accession_names_grm_id_prm_id_rep2{$a})) {
                    push @seen_accession_names_grm_id_prm_id_rep1_rep2, $a;
                }
            }

            open(my $F_grm_id_prm_id_gcorr, ">", $analytics_protocol_data_tempfile_grm_id_prm_id_gcorr) || die "Can't open file ".$analytics_protocol_data_tempfile_grm_id_prm_id_gcorr;
                print $F_grm_id_prm_id_gcorr "accession_name,g_rep1,g_rep2\n";

                foreach my $stock_name (@seen_accession_names_grm_id_prm_id_rep1_rep2) {
                    foreach my $trait_name (@sorted_trait_names) {
                        my $val1 = $result_blup_data_s_grm_id_prm_id_rep1->{$stock_name}->{$trait_name} || 'NA';
                        my $val2 = $result_blup_data_s_grm_id_prm_id_rep2->{$stock_name}->{$trait_name} || 'NA';
                        print $F_grm_id_prm_id_gcorr "$stock_name,$val1,$val2\n";
                    }
                }
            close($F_grm_id_prm_id_gcorr);
            ...
            foreach my $t_sec (@sorted_trait_names_secondary) {
                ...
                my $result_blup_data_s_grm_prm_sec_favg_rep1;
                my $genetic_effect_min_s_grm_prm_sec_favg_rep1 = 10000000000;
                my $genetic_effect_max_s_grm_prm_sec_favg_rep1 = -10000000000;
                my $genetic_effect_sum_s_grm_prm_sec_favg_rep1 = 0;
                my $genetic_effect_sum_square_s_grm_prm_sec_favg_rep1 = 0;
                my $result_residual_data_s_grm_prm_sec_favg_rep1;
                my $residual_sum_s_grm_prm_sec_favg_rep1 = 0;
                my $residual_sum_square_s_grm_prm_sec_favg_rep1 = 0;
                my $result_fitted_data_s_grm_prm_sec_favg_rep1;
                my $model_sum_square_residual_s_grm_prm_sec_favg_rep1 = 0;
                my @varcomp_original_grm_fixed_effect_grm_prm_sec_favg_rep1;
                my @varcomp_h_grm_fixed_effect_grm_prm_sec_favg_rep1;
                my @fits_grm_fixed_effect_grm_prm_sec_favg_rep1;
                my %seen_accession_names_grm_prm_sec_favg_rep1;

                my $prm_fixed_effects_grm_prm_sec_favg_rep1_cmd = 'R -e "library(sommer); library(data.table); library(reshape2); library(ggplot2); library(GGally);
                mat <- data.frame(fread(\''.$stats_tempfile.'\', header=TRUE, sep=\',\'));
                geno_mat_3col <- data.frame(fread(\''.$grm_file.'\', header=FALSE, sep=\'\t\'));
                sec_cont <- data.frame(fread(\''.$analytics_protocol_data_tempfile28.'\', header=FALSE, sep=\',\'));
                sec_binned <- data.frame(fread(\''.$analytics_protocol_data_tempfile30.'\', header=FALSE, sep=\',\'));
                ...
                "';
                print STDERR Dumper $prm_fixed_effects_grm_prm_sec_favg_rep1_cmd;
                my $prm_fixed_effects_grm_prm_sec_favg_rep1_cmd_status = system($prm_fixed_effects_grm_prm_sec_favg_rep1_cmd);
                    ...
                    while (my $row = <$fh>) {
                        ...
                        foreach my $encoded_trait (@header_cols_no_prm) {
                            if ($encoded_trait eq $trait_name_encoded_string) {
                                ...
                                my $value = $columns[$col_counter+1];
                                if (defined $value && $value ne '') {
                                    $result_blup_data_s_grm_prm_sec_favg_rep1->{$stock_name}->{$trait} = $value;
                                    $seen_accession_names_grm_prm_sec_favg_rep1{$stock_name}++;

                                    if ($value < $genetic_effect_min_s_grm_prm_sec_favg_rep1) {
                                        $genetic_effect_min_s_grm_prm_sec_favg_rep1 = $value;
                                    }
                                    elsif ($value >= $genetic_effect_max_s_grm_prm_sec_favg_rep1) {
                                        $genetic_effect_max_s_grm_prm_sec_favg_rep1 = $value;
                                    }

                                    $genetic_effect_sum_s_grm_prm_sec_favg_rep1 += abs($value);
                                    $genetic_effect_sum_square_s_grm_prm_sec_favg_rep1 = $genetic_effect_sum_square_s_grm_prm_sec_favg_rep1 + $value*$value;
                                }
                            }
                            $col_counter++;
                        }
                ...
                open($fh_residual, '<', $stats_out_tempfile_residual) or die "Could not open file '$stats_out_tempfile_residual' $!";
                    ...
                    while (my $row = <$fh_residual>) {
                        ...
                        my $stock_name = $plot_id_map{$stock_id};
                        if (defined $residual && $residual ne '') {
                            $result_residual_data_s_grm_prm_sec_favg_rep1->{$stock_name}->{$trait_name} = $residual;
                            $residual_sum_s_grm_prm_sec_favg_rep1 += abs($residual);
                            $residual_sum_square_s_grm_prm_sec_favg_rep1 = $residual_sum_square_s_grm_prm_sec_favg_rep1 + $residual*$residual;
                        }
                        if (defined $fitted && $fitted ne '') {
                            $result_fitted_data_s_grm_prm_sec_favg_rep1->{$stock_name}->{$trait_name} = $fitted;
                        }
                        $model_sum_square_residual_s_grm_prm_sec_favg_rep1 = $model_sum_square_residual_s_grm_prm_sec_favg_rep1 + $residual*$residual;
                    }
                close($fh_residual);
                ...
                open($fh_varcomp, '<', $stats_out_tempfile_varcomp) or die "Could not open file '$stats_out_tempfile_varcomp' $!";
                    ...
                    while (my $row = <$fh_varcomp>) {
                        ...
                        push @varcomp_original_grm_fixed_effect_grm_prm_sec_favg_rep1, \@columns;
                    }
                close($fh_varcomp);
                ...
                open($fh_varcomp_h, '<', $stats_out_tempfile_vpredict) or die "Could not open file '$stats_out_tempfile_vpredict' $!";
                    ...
                    while (my $row = <$fh_varcomp_h>) {
                        ...
                        push @varcomp_h_grm_fixed_effect_grm_prm_sec_favg_rep1, \@columns;
                    }
                close($fh_varcomp_h);
                ...
                open($fh_fits, '<', $stats_out_tempfile_fits) or die "Could not open file '$stats_out_tempfile_fits' $!";
                    ...
                    while (my $row = <$fh_fits>) {
                        ...
                        push @fits_grm_fixed_effect_grm_prm_sec_favg_rep1, \@columns;
                    }
                close($fh_fits);

                my $result_blup_data_s_grm_prm_sec_favg_rep2;
                my $genetic_effect_min_s_grm_prm_sec_favg_rep2 = 10000000000;
                my $genetic_effect_max_s_grm_prm_sec_favg_rep2 = -10000000000;
                my $genetic_effect_sum_s_grm_prm_sec_favg_rep2 = 0;
                my $genetic_effect_sum_square_s_grm_prm_sec_favg_rep2 = 0;
                my $result_residual_data_s_grm_prm_sec_favg_rep2;
                my $residual_sum_s_grm_prm_sec_favg_rep2 = 0;
                my $residual_sum_square_s_grm_prm_sec_favg_rep2 = 0;
                my $result_fitted_data_s_grm_prm_sec_favg_rep2;
                my $model_sum_square_residual_s_grm_prm_sec_favg_rep2 = 0;
                my @varcomp_original_grm_fixed_effect_grm_prm_sec_favg_rep2;
                my @varcomp_h_grm_fixed_effect_grm_prm_sec_favg_rep2;
                my @fits_grm_fixed_effect_grm_prm_sec_favg_rep2;
                my %seen_accession_names_grm_prm_sec_favg_rep2;

                my $prm_fixed_effects_grm_prm_sec_favg_rep2_cmd = 'R -e "library(sommer); library(data.table); library(reshape2); library(ggplot2); library(GGally);
                mat <- data.frame(fread(\''.$stats_tempfile.'\', header=TRUE, sep=\',\'));
                geno_mat_3col <- data.frame(fread(\''.$grm_file.'\', header=FALSE, sep=\'\t\'));
                sec_cont <- data.frame(fread(\''.$analytics_protocol_data_tempfile28.'\', header=FALSE, sep=\',\'));
                sec_binned <- data.frame(fread(\''.$analytics_protocol_data_tempfile30.'\', header=FALSE, sep=\',\'));
                ...
                "';
                print STDERR Dumper $prm_fixed_effects_grm_prm_sec_favg_rep2_cmd;
                my $prm_fixed_effects_grm_prm_sec_favg_rep2_cmd_status = system($prm_fixed_effects_grm_prm_sec_favg_rep2_cmd);
                    ...
                    while (my $row = <$fh>) {
                        ...
                        foreach my $encoded_trait (@header_cols_no_prm) {
                            if ($encoded_trait eq $trait_name_encoded_string) {
                                ...
                                my $value = $columns[$col_counter+1];
                                if (defined $value && $value ne '') {
                                    $result_blup_data_s_grm_prm_sec_favg_rep2->{$stock_name}->{$trait} = $value;
                                    $seen_accession_names_grm_prm_sec_favg_rep2{$stock_name}++;

                                    if ($value < $genetic_effect_min_s_grm_prm_sec_favg_rep2) {
                                        $genetic_effect_min_s_grm_prm_sec_favg_rep2 = $value;
                                    }
                                    elsif ($value >= $genetic_effect_max_s_grm_prm_sec_favg_rep2) {
                                        $genetic_effect_max_s_grm_prm_sec_favg_rep2 = $value;
                                    }

                                    $genetic_effect_sum_s_grm_prm_sec_favg_rep2 += abs($value);
                                    $genetic_effect_sum_square_s_grm_prm_sec_favg_rep2 = $genetic_effect_sum_square_s_grm_prm_sec_favg_rep2 + $value*$value;
                                }
                            }
                            $col_counter++;
                        }
                ...
                open($fh_residual, '<', $stats_out_tempfile_residual) or die "Could not open file '$stats_out_tempfile_residual' $!";
                    ...
                    while (my $row = <$fh_residual>) {
                        ...
                        my $stock_name = $plot_id_map{$stock_id};
                        if (defined $residual && $residual ne '') {
                            $result_residual_data_s_grm_prm_sec_favg_rep2->{$stock_name}->{$trait_name} = $residual;
                            $residual_sum_s_grm_prm_sec_favg_rep2 += abs($residual);
                            $residual_sum_square_s_grm_prm_sec_favg_rep2 = $residual_sum_square_s_grm_prm_sec_favg_rep2 + $residual*$residual;
                        }
                        if (defined $fitted && $fitted ne '') {
                            $result_fitted_data_s_grm_prm_sec_favg_rep2->{$stock_name}->{$trait_name} = $fitted;
                        }
                        $model_sum_square_residual_s_grm_prm_sec_favg_rep2 = $model_sum_square_residual_s_grm_prm_sec_favg_rep2 + $residual*$residual;
                    }
                close($fh_residual);
                ...
                open($fh_varcomp, '<', $stats_out_tempfile_varcomp) or die "Could not open file '$stats_out_tempfile_varcomp' $!";
                    ...
                    while (my $row = <$fh_varcomp>) {
                        ...
                        push @varcomp_original_grm_fixed_effect_grm_prm_sec_favg_rep2, \@columns;
                    }
                close($fh_varcomp);
                ...
                open($fh_varcomp_h, '<', $stats_out_tempfile_vpredict) or die "Could not open file '$stats_out_tempfile_vpredict' $!";
                    ...
                    while (my $row = <$fh_varcomp_h>) {
                        ...
                        push @varcomp_h_grm_fixed_effect_grm_prm_sec_favg_rep2, \@columns;
                    }
                close($fh_varcomp_h);
                ...
                open($fh_fits, '<', $stats_out_tempfile_fits) or die "Could not open file '$stats_out_tempfile_fits' $!";
                    ...
                    while (my $row = <$fh_fits>) {
                        ...
                        push @fits_grm_fixed_effect_grm_prm_sec_favg_rep2, \@columns;
                    }
                close($fh_fits);

                my @seen_accession_names_grm_prm_sec_favg_rep1_rep2;
                foreach my $a (sort keys %seen_accession_names_grm_prm_sec_favg_rep1) {
                    if (exists($seen_accession_names_grm_prm_sec_favg_rep2{$a})) {
                        push @seen_accession_names_grm_prm_sec_favg_rep1_rep2, $a;
                    }
                }

                open(my $F_grm_prm_sec_favg_gcorr, ">", $analytics_protocol_data_tempfile_grm_prm_secondary_gcorr) || die "Can't open file ".$analytics_protocol_data_tempfile_grm_prm_secondary_gcorr;
                    print $F_grm_prm_sec_favg_gcorr "accession_name,g_rep1,g_rep2\n";

                    foreach my $stock_name (@seen_accession_names_grm_prm_sec_favg_rep1_rep2) {
                        foreach my $trait_name (@sorted_trait_names) {
                            my $val1 = $result_blup_data_s_grm_prm_sec_favg_rep1->{$stock_name}->{$trait_name} || 'NA';
                            my $val2 = $result_blup_data_s_grm_prm_sec_favg_rep2->{$stock_name}->{$trait_name} || 'NA';
                            print $F_grm_prm_sec_favg_gcorr "$stock_name,$val1,$val2\n";
                        }
                    }
                close($F_grm_prm_sec_favg_gcorr);

                my $prm_fixed_effects_grm_prm_sec_favg_gcor_cmd = 'R -e "library(data.table);
                mat <- data.frame(fread(\''.$analytics_protocol_data_tempfile_grm_prm_secondary_gcorr.'\', header=TRUE, sep=\',\'));
                g_corr <- 0;
                try (g_corr <- cor(mat\$g_rep1, mat\$g_rep2, use = \'complete.obs\'));
                write.table(data.frame(gcorr = c(g_corr) ), file=\''.$stats_out_tempfile_gcor.'\', row.names=FALSE, col.names=TRUE, sep=\',\');
                "';
                print STDERR Dumper $prm_fixed_effects_grm_prm_sec_favg_gcor_cmd;
                my $prm_fixed_effects_grm_prm_sec_favg_gcor_cmd_status = system($prm_fixed_effects_grm_prm_sec_favg_gcor_cmd);
