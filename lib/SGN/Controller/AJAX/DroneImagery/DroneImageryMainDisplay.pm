...
sub raw_drone_imagery_summary_top_GET : Args(0) {
    ...
    while( my ($drone_run_band_project_id, $drone_run_band_project_name, $drone_run_band_project_desc, $drone_run_project_id, $drone_run_project_name, $drone_run_project_desc, $field_trial_project_id, $field_trial_project_name, $field_trial_project_desc, $drone_run_band_project_type, $drone_run_date, $drone_run_type, $drone_run_averaged_temperature_gdd, $drone_run_averaged_precipitation_sum, $drone_run_related_time_cvterm_json, $drone_run_indicator, $drone_run_phenotypes_indicator, $drone_run_processed, $drone_run_processed_extended, $drone_run_processed_vi, $drone_run_is_raw_images, $drone_run_ground_control_points_json, $drone_run_camera_rig, $drone_run_base_date, $drone_run_band_geoparam_coordinates, $drone_run_band_geoparam_coordinates_type, $drone_run_band_geoparam_polygons, $drone_run_odm_indicator, $private_company_id, $private_company_name) = $h->fetchrow_array()) {
        ...
        $unique_drone_runs{$field_trial_project_name}->{$drone_run_project_id}->{drone_run_odm_indicator} = $drone_run_odm_indicator;
        ...
        if ($drone_run_indicator) {
            $processing_field_trials_drone_runs_indicator{$field_trial_project_name}->{$drone_run_project_id}++;
        }
        if ($drone_run_phenotypes_indicator) {
            $processing_pheno_field_trials_drone_runs_indicator{$field_trial_project_name}->{$drone_run_project_id}++;
        }
        if ($drone_run_odm_indicator) {
            $processing_odm_field_trials_drone_runs_indicator{$field_trial_project_name}->{$drone_run_project_id}++;
        }
    ...
    foreach my $trial_name (sort keys %unique_drone_runs) {
        ...
        if (exists($processing_pheno_field_trials_drone_runs_indicator{$trial_name})) {
            $drone_run_html .= '&nbsp;&nbsp;&nbsp;&nbsp;<small>(Saving Phenotypes: <b>'.scalar(keys %{$processing_pheno_field_trials_drone_runs_indicator{$trial_name}}).'</b><span class="glyphicon glyphicon-hourglass"></span>)</small>';
        }
        if (exists($processing_odm_field_trials_drone_runs_indicator{$trial_name})) {
        ...
        foreach my $epoch_seconds (sort keys %{$unique_drone_run_dates{$trial_name}}) {
            ...
            if (exists($processing_field_trials_drone_runs_indicator{$trial_name}->{$k})) {
                $drone_run_html .= '&nbsp;&nbsp;&nbsp;&nbsp;<small>(Processing: <span class="glyphicon glyphicon-hourglass"></span>)</small>';
            }
            if (exists($processing_pheno_field_trials_drone_runs_indicator{$trial_name}->{$k})) {
                $drone_run_html .= '&nbsp;&nbsp;&nbsp;&nbsp;<small>(Saving Phenotypes: <span class="glyphicon glyphicon-hourglass"></span>)</small>';
            }
            if (exists($processing_odm_field_trials_drone_runs_indicator{$trial_name}->{$k})) {
                $drone_run_html .= '&nbsp;&nbsp;&nbsp;&nbsp;<small>(Running ODM: <span class="glyphicon glyphicon-hourglass"></span>)</small>';
            }

            $drone_run_html .= '</a></h4></div><div id="drone_run_band_accordion_drone_run_wrapper_one_'.$k.'" class="panel-collapse collapse"><div class="panel-body">';
            ...
            if (!$v->{drone_run_indicator}) {
                ...
                # else {
                    ...
                    $drone_run_html .= '<div class="panel-group" id="project_drone_imagery_buttons_sections_'.$k.'" ><div class="panel panel-default"><div class="panel-heading"><h4 class="panel-title"><a data-toggle="collapse" data-parent="#project_drone_imagery_buttons_sections_'.$k.'" href="#project_drone_imagery_buttons_sections_accordian_'.$k.'" >Additional Options</a></h4></div><div id="project_drone_imagery_buttons_sections_accordian_'.$k.'" class="panel-collapse collapse"><div class="panel-body">';
                        ...
                        $drone_run_html .= '<button class="btn btn-default btn-sm" name="project_drone_imagery_change_date_drone_run" data-drone_run_project_id="'.$k.'" data-drone_run_project_name="'.$v->{drone_run_project_name}.'" data-field_trial_id="'.$v->{trial_id}.'" data-field_trial_name="'.$v->{trial_name}.'" >Change Date</button><br/><br/>';
...
sub raw_drone_imagery_drone_run_band_summary_GET : Args(0) {
    ...
    my %original_background_removed_threshold_terms = (
        ...
        'threshold_background_removed_stitched_drone_imagery_coastal_blue' => 'Coastal Blue (410-480nm)',
        'threshold_background_removed_stitched_drone_imagery_green2' => 'Green (510-550nm)',
        'threshold_background_removed_stitched_drone_imagery_red2' => 'Red (590-670nm)',
        'threshold_background_removed_stitched_drone_imagery_red_edge2' => 'Red Edge (680-720nm)',
        'threshold_background_removed_stitched_drone_imagery_red_edge3' => 'Red Edge (720-760nm)',
    ...
    my %original_denoised_observation_unit_polygon_terms = (
        ...
        'Coastal Blue (410-480nm)' => ['observation_unit_polygon_coastal_blue_imagery'],
        'Green (510-550nm)' => ['observation_unit_polygon_green2_imagery'],
        'Red (590-670nm)' => ['observation_unit_polygon_red2_imagery'],
        'Red Edge (680-720nm)' => ['observation_unit_polygon_red_edge2_imagery'],
        'Red Edge (720-760nm)' => ['observation_unit_polygon_red_edge3_imagery'],
